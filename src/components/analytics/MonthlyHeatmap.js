    ...
    <div className="rounded-2xl bg-slate-800/50 border border-slate-700/50 p-6 backdrop-blur-sm">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
        {monthlyData.map((month, idx) => (
          ...
          >
            <div className={cn(
              "aspect-square rounded-lg flex flex-col items-center justify-center p-3 transition-transform hover:scale-105",
              getColor(month.pnl)
            )}>
            ...
            <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover:block">
              <div className="bg-slate-900 border border-slate-700 rounded-lg p-3 text-xs text-white whitespace-nowrap shadow-xl">
                ...
                <p className="text-slate-300">P&L: £{month.pnl.toFixed(2)}</p>
