  ...
  // ✅ Chronologically sorted trades (oldest → newest by exit date)
    ...
    const calculateTradeSharpe = () => {
      ...
      return stdDev > 0 ? avgReturn / stdDev : 0;
          ...
          // ✅ Use the same filtered portfolio history for BOTH Sharpe and Drawdown
      const filteredPortfolioHistory = getFilteredPortfolioHistory();
      
      // ✅ Sharpe (filtered)
      let sharpeRatio = 0;
      let sharpeMethod = filteredPortfolioHistory.length >= 1 ? "portfolio" : "trade";
      
      if (filteredPortfolioHistory.length >= 30) {
        const sorted = [...filteredPortfolioHistory].sort(
