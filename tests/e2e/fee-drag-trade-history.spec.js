 ...
 *   (null → text-slate-400 "—", non-null → text-amber-400 "+X.XX%")
 */
...
test.describe('SC-FEE-04 — Null fee drag shows em dash', () => {
  ...
  test('SC-FEE-04a: Trade without fee drag shows em dash in slate colour', async ({ page }) => {
    // BARC has fee_drag_pct: null → formatFeeDrag → "—", class → text-slate-400
    const barcRow = page.locator('tr').filter({ hasText: 'BARC' });
    await expect(barcRow).toBeVisible();
    // BARC has null fee_drag, slippage, and R-multiple — all show '—' in text-slate-400.
    // Use .first() since any em-dash in slate-500 on this row confirms null renders correctly.
    const feeDragCell = barcRow.locator('.text-slate-400').filter({ hasText: '—' }).first();
