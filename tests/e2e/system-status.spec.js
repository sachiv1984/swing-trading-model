...
test.describe('SC-SS-01 — Pre-run state', () => {
  ...
  test('SC-SS-01b: Pre-run state shows "92 endpoints" placeholder', async ({ page }) => {
    // Before running tests, the page shows: "Tests 92 endpoints"
    // (totalTests || '92' → '92' before any test run; corrected to match backend/routers/test.py
    // test_cases count of 92 after v7.5 EPIC-02 ST-02 added GET/POST /price-alerts and
    // DELETE /price-alerts/{id} (BLG-FE-116, custom price alerts))
    await expect(page.getByText(/tests 92 endpoints/i)).toBeVisible({ timeout: 8000 });
  });
