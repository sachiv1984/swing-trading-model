...
test.describe('SC-SS-01 — Pre-run state', () => {
  ...
  test('SC-SS-01b: Pre-run state shows "41 endpoints" placeholder', async ({ page }) => {
    // Before running tests, the page shows: "Tests 41 endpoints"
    // (totalTests || '41' → '41' before any test run)
    await expect(page.getByText(/tests 41 endpoints/i)).toBeVisible({ timeout: 8000 });
  });
