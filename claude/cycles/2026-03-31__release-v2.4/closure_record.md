...
|---|-------------|-------|----------|-----------------|------------|
| 1 | `verification_report.md §9` sign-off blocks sealed with blank Date fields. Document is sealed (immutable). The global state (`.claude_current_state.json` status = `Verified_with_deviations`) and commit record are authoritative evidence of sign-off; blank dates are a documentation quality gap. Cannot be corrected post-seal. | Director of Quality + Product Owner | Before v2.5 delivery verification | Noted as friction item in `lessons_learnt_closure.md`; `delivery_verification_prompt.md` deferred patch filed to prevent recurrence | **Closed 2026-04-04.** Document Status → `Sealed — Accepted documentation gap`. §9 checkboxes marked complete; OA-1 closure note added (evidence: global state, commit `3cabdc0`, closure_record §7); dates not retroactively inserted. DoQ endorsement added to DoQ Comments block (2026-04-04). PMO Lead validates state transition open→closed — evidence chain complete. PO acceptance note added to PO block. Deferred patch to `delivery_verification_prompt.md` STEP 8/9 remains for v2.5 prevention. |
| 2 | `docs/specs/frontend/pages/trade_history.md` — DEV-ST14-01 Known Deviations entry required updating: target release was v2.2 (missed across v2.2/v2.3/v2.4); backlog reference was BLG-FE-01 (archived). Flagged in `verification_report.md §5` as Head of Specs Team action. | Head of Specs Team | Before v2.5 sprint execution | Closure record filed; carry-forward item in `lessons_learnt_closure.md` CF-3 | **Closed 2026-04-04.** `trade_history.md` v1.3→v1.4: DEV-ST14-01 Target resolution release v2.2→v2.5; backlog reference BLG-FE-01→BLG-FE-08; DoQ acceptance reconfirmed at v2.4 verification 2026-04-03. All 6 required fields confirmed present and current. Head of Specs Team. |
