...
**Post-Merge Follow-Up (required before AC3 on ST-07 is considered fully closed at the workflow-execution level, not just the shell-semantics level):** Run `gh workflow run staging-deploy-drift-check.yml --ref main` (or equivalent) immediately after this EPIC merges, and confirm the run completes with the correct `exit_code` output. This is tracked as an open action for the sprint close phase, not deferred indefinitely.
