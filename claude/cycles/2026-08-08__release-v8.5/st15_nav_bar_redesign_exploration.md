...
**Status:** Complete
**Last Updated:** 2026-08-11 (ST-09, EPIC-03, v8.6, BLG-FE-154 — corrected group/page counts against live `NAV_GROUPS`, which had grown since this doc's 2026-08-10 counts)
...
- **Structure:** left sidebar, 4 collapsible groups (Trading — 5 items, Analytics — 5, Tools — 3, System — 4 — 17 items total) + a top-level `DashboardHome` link outside any group (18 curated nav entries in all). `docs/specs/frontend/pages/navigation.md` §Group Structure is the canonical spec. **2026-08-11 correction (ST-09, BLG-FE-154):** the 2026-08-10 counts above (Trading 4/Analytics 3/Tools 1, "13 nav items") had already drifted from the live `NAV_GROUPS` array in `src/Layout.js` by the time this doc's own sign-off date — Analytics gained Strategy Benchmark, Risk Dashboard, Signals, Reports; Trading gained Trade History, Reflections, Red Flag Journal; Tools gained Watchlist, Ticker Universe. Counts corrected to match `NAV_GROUPS` exactly; the underlying evaluation and recommendation below are unaffected (still well within a curated set, not a flat 20-item list).
- **Collapse behaviour:** only the active group is expanded by default; all others start collapsed and persist their state per-session (`sessionStorage`) — the sidebar does not show all 17 grouped nav items' full label+icon rows simultaneously except when a user deliberately expands every group.
