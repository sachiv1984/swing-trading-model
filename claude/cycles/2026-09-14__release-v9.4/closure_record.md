...
|---|-------------|-------|----------|-----------------|------------|
| 1 | Ready-pool-vs-capacity gap has widened for 2 consecutive cycles (61→74 items unselected growth) — assess whether the capacity ceiling should rise or a P3 sub-tiering pass is warranted. | PMO Lead / Head of Specs Team | Next scheduled roadmap rebalance | Standard rebalance review | **Resolved 2026-09-15** — `roadmap_prompt.md` v9.20→v9.21, new §7.3 Ready-Pool Capacity Gap Trend codifying a mandatory decision point at 3 consecutive widening readings; current reading (2 consecutive) below threshold, no capacity/sub-tiering change made, decision recorded in-prompt per explicit user direction. |
| 2 | `document_lifecycle_guide.md §9` should require a `DEV-<id>` on every `## Known Deviations` entry from point of filing — 2 entries filed since the last consolidation review (`BLG-FE-172`, `BLG-BE-112`) are field-complete but carry no ID, invisible to the review's heading-based scan. | Head of Specs Team | Before the next Cross-Cycle Deviation Consolidation Review (due in 3 cycles) | File a `BLG-GOV-*` backlog item | **Resolved 2026-09-15** — `document_lifecycle_guide.md` v2.7→v2.8, §9 gains a required `DEV-<id>` field; both non-compliant entries retroactively assigned (`DEV-v9.1-ST13-01`, `DEV-v9.3-ST03-01`/`-02`). |
| 3 | `release_planning_prompt.md` §1.4c — canonical over-capacity ready-pool selection method still not codified (carried from `v9.3`, 1 cycle carried). | Head of Specs Team | Next `release_planning_prompt.md` revision touching §1.4 | Standard prompt revision | **Resolved 2026-09-15** — `release_planning_prompt.md` v2.51→v2.52, new §1.4c codifying the category-balanced/P2-first/oldest-first method. |
| 4 | `shared_standards.md §16.4`/`execution_prompt.md` — mid-sprint surfacing of open, SLA-breached, non-blocking escalations still not added (carried from `v9.3`, 1 cycle carried). | Head of Specs Team | Next revision touching escalation SLA tracking | Standard prompt revision | **Resolved 2026-09-15** — `execution_prompt.md` v3.75→v3.76 (new STEP -1.2A) + `shared_standards.md` v3.33→v3.34 (new §16.4.1). |
| 5 | Deviation consolidation review's structural-fix recommendation (require a resolving commit to also update the canonical spec's own labeled Known Deviation fields) remains unfiled as a backlog item across the 4th and 5th runs (no new drift instance this window, so not re-escalated with new urgency). | Head of Specs Team | Before a 4th confirmed drift instance accrues | File a `BLG-GOV-*` backlog item | **Resolved 2026-09-15** — filed `BLG-GOV-332`. |

**All 5 outstanding actions resolved 2026-09-15**, same day as closure, per explicit user direction ("fix the 5 outstanding actions") — acting as Head of Specs Team (items 2–5) and jointly with PMO Lead (item 1). See `claude/system/prompt_change_log.md` 2026-09-15 rows for full per-file detail and `claude/backlog/backlog.md#BLG-GOV-332` for item 5.
