...
- **Status:** In Progress — sign-off cleared (Head of Engineering resolved portfolio_id scoping and finalised the migration; Data Model & Domain Schema Owner agent-mediated Approved after 1 Blocked retry — see DS-17 sign-off block in `data_model.md`). Awaiting commit-SHA write on push before terminal `Unblocked`.
