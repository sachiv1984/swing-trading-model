...
|---------|----------------|-----------------|----------------------|--------|------------|
| ST-27 | `release_planning_prompt.md` §1.3a (v2.54); `scripts/scan_backlog_gate_conditions.py` | Scan now reports a separate "date-lapsed — verify" list; §1.3a requires it be read and each item cleared or re-gated before the ready pool is fixed | AC-01 (scan reports separately) — done. AC-02 (6 named items verified, cleared or re-gated) — **partial**, see notes below. AC-03 (§6 checklist complete) — done | Pass with notes | None (disclosed limitation) |
...
1. **ST-27 AC-02 is disclosed-partial, not fully met, and this is a known, pre-flagged constraint — not an oversight.** `sprint_backlog.md`'s own Outstanding Actions table records this exact conflict at planning time: `execution_prompt.md` §7 does not permit the engine to edit existing `claude/backlog/backlog.md` items (new-item addition only), so the 4 items still requiring disposition (`BLG-FEAT-59`/`60`/`63`/`BLG-FE-84` — `BLG-GOV-90`/`BLG-GOV-188` were already handled via ST-31/ST-32 per the sprint backlog's own note) could not be re-gated in `backlog.md` directly. The determination (re-gate, anchored to the already-scheduled 2026-09-24 AI review) and the exact replacement text for each item's `Gate criteria` line are recorded in `docs/product/decisions/decisions--2026-09-21__release-v9.6.md`, for Head of Specs Team / Product Owner to apply. Until applied, these 4 backlog items still carry their stale `~2026-07-25` gate text.
