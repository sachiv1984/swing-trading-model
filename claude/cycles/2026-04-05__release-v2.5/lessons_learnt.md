...
| OA-03 | Hook configuration fix: review user-prompt-submit-hook write target before sprint execution | User / Infrastructure & Operations Owner | Before sprint execution begins | **Closed 2026-04-05** — Review confirmed: no `user-prompt-submit-hook` is configured in either `~/.claude/settings.json` (global) or `.claude/settings.local.json` (project). Problematic hook was already removed; no corrective action required. Standing constraint recorded: if a `UserPromptSubmit` hook is added in future, its write target must not be any path under `claude/backlog/`, `claude/cycles/`, `claude/system/`, or any other governed artefact path. |
