...
- `tests/test_doq_signoff_staleness_check.py` (5 new unit tests, +1 post-PR-open regression test — see note below)
...
| ST-18 | `scripts/check_doq_signoff_staleness.py`, `.github/workflows/quality_gate.yml` | Pre-merge lint catching residual "Pending DoQ"/"Awaiting QA" placeholders in the active cycle's qa_evidence files (catches the CLAUDE.md §8 cross-EPIC-merge scenario) | Lint check added to quality_gate.yml; fails on a synthetic Pending-row test case | Pass | None — a real-CI self-referential false positive was found and fixed post-PR-open (see note below), not shipped |
...
**Post-PR-open CI failure and fix (2026-08-07):** after PR #1260 opened, `DoQ Sign-off Staleness Lint (ST-18)` failed in real CI. Root cause: the lint's own detection regex (`Pending DoQ|Awaiting QA`) matched *any* occurrence of those two phrases anywhere in a qa_evidence file — including ST-18's own row in this very file, whose "What was built" cell quotes both phrases as prose describing what the check catches. This is a self-referential false positive, not an actual stale placeholder — `qa_evidence_template.md`'s own convention (line 34) defines the placeholders as a **standalone Result-column cell value**, and ST-18's own Result cell already correctly read `Pass`. Fixed by anchoring the regex to require the placeholder occupy its own table cell (`\|\s*(?:Pending DoQ|Awaiting QA)\s*\|`) rather than matching anywhere in a line, generalising the template's actual documented placeholder-usage convention into the detection logic. Re-verified: all 6 unit tests pass (5 original + 1 new regression test for this exact false-positive class, `test_prose_quoting_placeholder_strings_is_not_flagged`), and a live run of `scripts/check_doq_signoff_staleness.py` against the real cycle now reports 0 findings across all 3 EPIC qa_evidence files. Ironic given the story's own subject matter, but caught and corrected before merge — not shipped as a deviation.
