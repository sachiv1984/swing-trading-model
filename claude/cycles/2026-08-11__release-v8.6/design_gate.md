...
- **ST-07's design decision recommends a follow-up implementation item** (not filed by this gate — `claude/backlog/backlog.md` is outside this routine's §5 write scope; PMO Lead/Product Owner to file separately) — the decision record itself does not ship the 4 hardcoded-modal fixes; that is deliberately separate scope, to be sequenced after `BLG-FE-147`/ST-04 (this cycle) per the token-registration dependency noted in the decision record.
