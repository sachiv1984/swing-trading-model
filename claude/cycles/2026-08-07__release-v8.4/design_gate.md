...
**Follow-up filed:** `BLG-GOV-288` — Release Planning STEP 0 (or an equivalent early step) should explicitly reset `.claude_current_state.json` `sprint_sealed = false` on publishing a new cycle's release plan, so the root pointer doesn't silently carry a stale `true` from the prior cycle's seal into the next cycle's Design Gate / Sprint Planning preconditions. Not filed as part of this routine's own write (this engine's §5 write-scope restriction prohibits modifying any backlog document during a design-gate run) — filed via a separate `/backlog-add` invocation after this gate record was committed, per explicit user instruction ("file the backlog").
