...
- **CI false positive, not a required check (PR #1294):** "PII Field-Name Pattern Scan (ST-08)" flagged `email_enabled` (in the pre-existing `NotificationPreferences`/`UpdateNotificationPreferencesRequest` schemas) as newly-added. Confirmed byte-identical to `origin/main` at the same content — not new. Root cause: ST-02's structural fix relocates a ~1166-line block within `openapi.yaml`, which shifts every unrelated line after the insertion point later in the file; `git diff --unified=0` (no move-detection) renders this as delete-at-old-position + add-at-new-position rather than recognising unchanged content. Confirmed via `gh api repos/.../branches/main/protection` that this check is **not** in `required_status_checks` — does not block merge. Documented on the PR per the script's own guidance ("If a false positive, note why in the PR").
