...
**Real CI outcome (confirmed, PR #1387):** All 4 SC-TOK scenarios passed in real GitHub Actions CI (`Playwright E2E Acceptance Tests 1/4–4/4`, all green, run 31681930191). The LL-v8.3-P3-02 precedent held in the other direction than its worst case here — the manual static trace was accurate for all 4 scenarios. **Separately, `test_tax_year_boundary_completeness.py`'s new ST-09 test (locally-verified passing above) failed in the real CI run** (`Pytest Phase B — real Postgres service`, 1 failed/1102 passed) with a different, unrelated cause: it relied on `reports_service.get_trade_history_by_tax_year`'s ambient import-time binding remaining the real (unmocked) function, which is not guaranteed across a shared 1100+-test session — confirmed 0 rows returned in CI vs 1 expected, where local isolated runs always returned 1. Fixed by explicitly rebinding it to `database.get_trade_history_by_tax_year` inside the test's own patch scope (commit `0d135715`), making it correct regardless of suite-wide test ordering. This is exactly the class of gap real-CI confirmation exists to catch that a sandboxed/isolated pass cannot — see commit message for full detail.
...
- Date: 2026-08-13
- Comments: Real CI run on PR #1387 confirms all 4 SC-TOK Playwright scenarios green (`Playwright E2E Acceptance Tests 1/4–4/4`, run 31681930191) — the static manual trace done in this sandbox (Playwright unsupported on this sandbox's OS, `ubuntu26.04-x64`) was accurate for all 4. Separately, the same real CI run caught a genuine test-isolation bug in ST-09's new pytest (passed locally in every isolated run, failed in the full 1103-test Phase B suite) — fixed in commit `0d135715` and awaiting final re-run confirmation. This is the intended outcome of the LL-v8.3-P3-02 real-CI-confirmation requirement: it caught exactly the class of gap a sandboxed/isolated pass cannot.
