...
| ST-20 | `docs/ops/test_environment_parity_check_2026-08-16.md` | Compared local/CI/staging across Python version, Node version, DB engine, frontend env vars. Real drift found: `backend/.python-version` correctly pins 3.11.0 (matching CI's 21 occurrences across 16 workflow files and staging's `render.yaml`), but this session's local `backend/.venv` runs 3.14.4 anyway — the pin exists but isn't honoured (no `pyenv`/version-manager shim on `PATH` to read it). DB engine and Node version confirmed consistent. `.env.production` missing `PUBLIC_URL` present in staging/`render.yaml` — flagged as advisory-only (production env vars are dashboard-only, not repo-visible, so unconfirmed rather than asserted as a defect). Production itself explicitly out of repo-comparable scope (disclosed, not silently skipped). **Corrected after QA Lead first-pass Blocked:** original draft wrongly claimed no `.python-version` file existed at all (search never covered `backend/`) and undercounted the CI grep hits (~4x). | Audit complete; drift fixed or documented as intentional; QA Lead sign-off | Pass | None — gap filed (`BLG-OPS-146`, rescoped to the corrected root cause), not a spec deviation |
