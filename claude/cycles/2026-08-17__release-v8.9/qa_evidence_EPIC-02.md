...
- Known deviations: None found.
- **Post-open PR review fix (commit `25ba1686`, 2026-08-19):** an independent agent-mediated Director of Quality review of PR #1453 found `concentration_reason`'s text mislabeled the sector's share of total portfolio value as `"% of portfolio heat"` — a distinct, already-defined metric on the same endpoint (`heat_impact_percent`, ST-05). `current_pct = sector_value / total_value * 100` is a value-concentration ratio, unrelated to the heat/risk calculation. Corrected to `"% of portfolio value"` in both the reduce-triggered and flag-only reason strings (`sizing_service.py`), the two test files that had locked in the wrong wording (`test_sizing_concentration.py`, `position-sizing-concentration.spec.js`), and `portfolio_endpoints.md`'s example/field-note text (v2.8.0→v2.8.1). Full backend suite re-confirmed green (1195 passed, 5 skipped, 0 failed) and the affected Playwright spec re-run clean (3/3 passed) after the fix.
