...
**QA test coverage:**
- Scenarios run: `tests/e2e/notification-badge-contrast.spec.js` (SC-BC-01 collapsed-header badge resolves to red-600; SC-BC-02 item-level badge resolves to red-600). Written against Tailwind's resolved `rgb()` computed-style values (hex-to-rgb math verified: red-600 = rgb(220, 38, 38)). **Not locally executed**: this sandbox's OS (ubuntu26.04) is unsupported by the installed Playwright version, blocking local chromium binary install (same limitation encountered and disclosed for EPIC-04). Will run in CI (`.github/workflows/playwright.yml`) at PR open.
