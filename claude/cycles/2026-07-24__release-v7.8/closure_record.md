...
|---|-------------|-------|----------|-----------------|------------|
| 1 | Confirm which engine's prompt owns `.claude_current_state.json.next_release` maintenance and make that ownership explicit in the owning prompt — field was found 4 releases stale at the start of this cycle's Release Planning session. | Head of Specs Team | Next governance prompt maintenance pass | Standard governance prompt maintenance | ✅ Resolved 2026-07-27 — `release_planning_prompt.md` v2.44 STEP 9 made the sole authoritative writer of `next_release` (unconditional write from `--version` on every seal); `roadmap_prompt.md` v9.5 STEP 8 advisory text now explicitly non-authoritative. See `prompt_change_log.md` 2026-07-27 rows. |
| 2 | Convert `execution_prompt.md` §3.2.B's API performance baseline pre-PR check from prose advisory to an enforced pre-commit/pre-PR-open script step — has now failed to prevent the same class of miss twice (v7.6/EPIC-07, v7.8/EPIC-06). | Head of Specs Team | Next run of Sprint Execution | Standard governance prompt maintenance | ✅ Resolved 2026-07-27 — `execution_prompt.md` v3.60 §3.2.B now runs `scripts/check_api_performance_baseline_drift.py` (hard exit code) instead of a prose `grep` instruction; `quality_gate.yml`'s CI job delegates to the same script. |
| 3 | Decouple `backend/services/changelog_digest_service.py`'s import chain from `backend/services/__init__.py`'s full package import (transitively requires live `DATABASE_URL`) so the digest script can run standalone in a DB-less sandbox. | Head of Engineering | Next run of Sprint Execution touching this file | Standard Sprint Execution backlog | ✅ Resolved 2026-07-27 — `changelog_digest_service.py` and `scripts/send_changelog_digest.py` now load `si05_digest_service.py` / `changelog_digest_service.py` directly via `importlib` file-path loading, bypassing `services/__init__.py`. Verified: `scripts/send_changelog_digest.py` runs with `DATABASE_URL` unset; `tests/test_changelog_digest_service.py` (10 tests) still pass. |
| 4 | Decision: apply the deferred structural fix for the `execution_state.json` cross-EPIC conflict pattern (proactive per-branch rebase step at STEP 2/4), or explicitly accept the manual-resolve cost as standing for this sprint's multi-EPIC-parallel-branch model. Recurrence: 2nd consecutive cycle (v7.7→v7.8), cost scaled up (11/12 branches vs 10/11). | Head of Specs Team | 2026-07-30 (72h) | Recurrence escalation per `lessons_learnt_prompt.md` §3.7 | ✅ Resolved 2026-07-27 — ruling: apply the fix (deferring a 3rd time was already flagged as unacceptable per `lessons_learnt_closure.md` row 1). `execution_prompt.md` v3.60 STEP 4 adds step 3c: proactive per-branch `execution_state.json` sync onto every pending sibling immediately after each merge. |
| 5 | Decision: apply the deferred structural fix for endpoint-count/hardcoded-constant fallback collisions across sibling PRs (flag AST-derivable values for mandatory re-derivation at rebase time), or accept the recurring risk. Recurrence: 2nd consecutive cycle (v7.7→v7.8), identical shape. | Head of Engineering | 2026-07-30 (72h) | Recurrence escalation per `lessons_learnt_prompt.md` §3.7 | ✅ Resolved 2026-07-27 — ruling: apply the fix. `execution_prompt.md` v3.60 §3.1.A step 3 adds a mandatory re-derivation check for any AST-derivable hardcoded constant, before commit and again before PR-open if `main` has moved. |
| 6 | Decision: rule on whether agent-mediated "acting as Product Owner"/"acting as Director of Quality" PR review comments should be disallowed entirely, and if permitted, under what labeling convention. Carried unruled from v7.7 Phase 3; this cycle worked around the risk via an owner-confirmed labeling convention (agent-mediated, blank sign-off fields for human completion) rather than a codified rule. | Head of Specs Team | 2026-07-30 (72h) | Authority ambiguity, carried forward 2nd cycle | ✅ Resolved 2026-07-27 — ruling: permitted, not disallowed, under a mandatory labeling convention. `execution_prompt.md` v3.60 §5.1 codifies: never label as the human role itself; always label as agent-mediated, on behalf of the named role; sign-off fields remain blank until human-completed. |
