...
- Frontend: `tests/e2e/whats-new-panel.spec.js` — 5 scenarios: SC-WN-01 (ready state, version header + ordered bullets), SC-WN-02 (11 changes → 8 bullets + "+3 more"), SC-WN-03 (empty state), SC-WN-04 (error state, custom heading), SC-WN-05 (loading spinner). Written and reasoned through against the actual component/DataState logic, but **not locally executed**: this sandbox's OS (ubuntu26.04) is unsupported by the installed Playwright version, blocking local chromium install (same limitation disclosed for every frontend EPIC this sprint — EPIC-03, EPIC-04). Will run in CI (`playwright.yml`) at PR open.
