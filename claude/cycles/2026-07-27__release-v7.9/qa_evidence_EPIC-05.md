...
**Post-push CI failure and fix — missing dependency (2026-07-28):** CI's "Playwright E2E Acceptance Tests" and "Playwright Visual Snapshots" both failed on the full run — not just this story's own 5 tests, all 666 tests in the suite failed. Root cause: `src/components/ui/tooltip.js` (pre-existing on `main`, added well before this sprint) imports `@radix-ui/react-tooltip`, but that package was never added to `package.json` or installed. Nothing had ever actually rendered that shared component until `TrailingStopExplainerIcon.js` did, so the dependency gap was latent and undetected until this story exercised it — the resulting webpack "Module not found" broke the dev server's compile entirely, taking down every route the E2E suite hits, not just Positions. This is a pre-existing repo gap, not introduced by this story's own code. Fixed by adding `"@radix-ui/react-tooltip": "^1.2.16"` to `package.json` (matching the versioning convention of this repo's other `@radix-ui/*` dependencies) and regenerating `package-lock.json` via `npm install --legacy-peer-deps` (CI's own install command). Verified the module now resolves (`require.resolve('@radix-ui/react-tooltip')` succeeds); a full `react-scripts build` could not be run in this sandbox (OOM-killed, exit 137 — a sandbox resource limitation, not a build failure) so final confirmation relies on CI's next run.
