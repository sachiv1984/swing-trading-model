...
**Source prompts:** `claude/system/roadmap_management_prompt.md` (v1.4), `claude/system/backlog_management_prompt.md` (v1.14), `claude/system/ideas_housekeeping_prompt.md` (v1.2)  
...
| 4.162 | 2026-08-17 | **Sprint execution `2026-08-14__release-v8.8` EPIC-07/ST-29 (BLG-GOV-293) — post_ship_closure.md v2.26→v2.27: STEP 10 named the sole authoritative writer of `.claude_current_state.json.prior_cycle`.** §10 source prompt header v2.26→v2.27 (line 1062). §14 Post-Ship Closure Engine v2.26→v2.27. §14 Version 4.161→4.162/2026-08-17. Change: `prior_cycle` had no owning engine at all (found stale at `2026-08-08__release-v8.5` release planning, mirroring the OA-1 `next_release` gap) — STEP 10 now writes this run's own `<cycle_id>` unconditionally, chosen over `release_planning_prompt.md` STEP 9 because by STEP 9 `active_cycle` has already advanced to the new cycle, losing the old value. **Known cross-branch version-collision risk (CLAUDE.md §8 step 2a):** this cycle's `EPIC-03` branch independently bumped this same v4.161→v4.162/2026-08-13 slot for an unrelated change (ST-13 changelog service fix) before this branch diverged — confirm at merge time which change actually owns v4.162 and renumber the later-merging branch's bump per §8 step 2a before committing the resolution. Authority: Head of Specs Team (Sprint Execution Engine, agent-mediated, ST-29, 2026-08-17). |
