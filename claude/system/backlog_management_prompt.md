...
**Status:** Active
**Version:** 1.18
**Last Updated:** 2026-09-19 (post-ship closure `2026-09-15__release-v9.5` follow-up, Head of Specs Team direct action — STEP 1.5 Type 4 now also matches idea-intake `## Idea Intake <window> — Promoted-Backlog Disposition` staging sections; STEP 6.2 gains a section-heading-preservation check); prior — 2026-09-09 (post-ship closure 2026-09-07__release-v9.2 outstanding-actions resolution, Head of Specs Team direct action — §1.2 gains a Gate-Inheritance Field-Completeness Scan, LL-v9.2-Closure-01, closing the BLG-FEAT-92 4-cycle manual-reconciliation friction item); prior — 2026-09-08 (ST-33, EPIC-04, v9.2, BLG-GOV-272 — new §3.1 Recurring Spec-Debt Deep Review Cadence, every 3rd invocation, tracked via a backlog.md header marker); prior history retained — see prior entries in version control.
...
**Gate-Inheritance Field-Completeness Scan (LL-v9.2-Closure-01):** Distinct from the two checks above (which look for missing fields on the item itself), this scan looks for a missing field on an item's *own* `**Gate criteria:**` when that item is instead excluded from scope solely because it inherits another item's gate. Pattern to detect: a `### BLG-xx` entry whose body references another backlog item as the reason it is out of scope (e.g. "reconciled sub-scope of `BLG-yyy`", "excluded per the `BLG-yyy` reconciliation", or similar cross-reference language) but which itself carries no `**Gate criteria:**` field of its own. For each such item found: flag it in the health summary under a **Gate-Inheritance Gaps** subsection, and (with the named owner's confirmation, not automatically) add a `**Gate criteria:**` field that states the inheritance explicitly, e.g. `**Gate criteria:** Inherits BLG-yyy's gate (see BLG-yyy) — track disposition there.` Do not silently apply the same fix to `BLG-yyy` itself; this scan only concerns items that reference another's gate without stating their own. If no candidates found: note "Gate-Inheritance Field-Completeness Scan: PASS — 0 candidates found" in the health summary. Closes a friction item first surfaced at `2026-09-07__release-v9.2` post-ship closure (Release Planning Friction Item 1): `BLG-FEAT-92`'s exclusion depended entirely on a Product Owner decision recorded in a *different* item's decisions document, requiring the same manual re-lookup across 4 consecutive cycles (v8.9–v9.2) because no structural field on `BLG-FEAT-92` itself named the dependency.
...
4. **Roadmap Rebalance / Delivery Verification idea-intake "New Items" sections** (added AUD-2026-08-12-003, closing a gap where 4 such sections dated `2026-07-24__scheduled` through `2026-08-11__scheduled` accumulated unpromoted across 4+ groom runs): any section headed `## Roadmap Rebalance <date>__scheduled — New Items (...)` or `## Delivery Verification <date>__release-v<x.y> — New Items`. **Also matches (v1.18):** any `## Idea Intake <window_id> — Promoted-Backlog Disposition (...)` heading, written by idea intake's direct-to-backlog disposition. That variant matched neither pattern above and persisted unrelocated across two consecutive `groom backlog` runs (`2026-09-14`, `2026-09-15`) while holding ~60 items — found at `2026-09-15__release-v9.5` closure. Unlike types 1–3, items in this section carry no completion marker of their own — the section is a staging area for newly-added items, not a completion-tracked list, so it has no "all resolved" condition to wait for.
...
**Section-heading preservation (v1.18):** an item block's boundary is the next `### ` heading, but a `## ` section heading (and its intro paragraph) can sit between two item blocks and belong to neither. When removing or relocating item blocks by splitting on `### `, that heading is swallowed as trailing text of the block before it. Before finishing, run `git diff claude/backlog/backlog.md | grep '^-## '` and confirm every removed `## ` heading was removed on purpose (an ephemeral section per STEP 1.5). Found live at `2026-09-15__release-v9.5` closure: archiving `BLG-FE-176` silently deleted the `## Idea Intake IW-20260914-01` heading that followed it; caught and restored the same session.
