...
If `completed_cycle_count % 2 == 1` (i.e., odd):
  ...
  - **If `next_release` equals this run's own release version** (v2.34): Release Planning STEP 9 stamps `next_release` from its own `--version` argument at seal and nothing advances it afterwards, so at post-ship time it names the release *being closed*, not an unplanned successor. Treat it as `[TBD]`/unscoped for this check and record the same advisory as the `[TBD]` branch below: "⚠ REBALANCE SKIP WITHHELD — completed_cycle_count = N (odd), but `next_release` (`v<X.Y>`) is the release just shipped, so no successor is scoped. A rebalance or scoping decision is needed before `plan release` can proceed — recommend `run roadmap --reason scheduled` despite the odd cadence." Found live at `2026-09-15__release-v9.5` closure: `next_release` read `v9.5` while closing v9.5 with roadmap §1 "Next planned release" = `[TBD]`, which fell through to the third branch and would have advised `plan release v9.5` for an already-shipped release.
...
**Markdown-formatting normalisation (required before diffing, v2.33 — LL-v9.2-P-Closure-01):** Separately from path-parameter naming, the two source documents also do not share a consistent Markdown presentation for an endpoint identifier — `openapi.yaml` is structured YAML with no ambiguity, but `api_performance_baseline.md`'s endpoint table rows may or may not wrap the `METHOD /path` string in backticks, and may append an example query string to the same cell (e.g. `GET /analytics/metrics?period=all_time` for the bare `GET /analytics/metrics` endpoint). Before diffing: strip any surrounding backticks and any `?...` query-string suffix from every endpoint identifier extracted from either document, reducing each to a bare `METHOD /path` string, then apply the path-parameter normalisation above. Skipping this step produces false-positive gaps of exactly the same shape as the path-parameter case — found live at `2026-09-07__release-v9.2` closure, where a first-pass parse assuming backtick-wrapped, query-string-free rows produced an apparent 80-endpoint gap against the real count of 0.
