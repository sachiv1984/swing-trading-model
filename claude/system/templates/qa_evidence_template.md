...
>
> **Disambiguation (added v1.15, LL-v9.4-P4-01):** `Pass, escalation open` is reserved for a named, open `ESC-*` record only. If a story's AC is fully met and the work merely surfaces a new, unrelated backlog item during its own execution (an incidental finding, not a tracked escalation), use plain `Pass` (or `Pass with notes`) with the backlog item cited in the `Deviations`/Comments cell instead — do not use `Pass, escalation open` for a backlog-item reference. Found live at `2026-09-14__release-v9.4`: `qa_evidence_EPIC-01.md`'s ST-05 row used `Pass, escalation open` but named `BLG-OPS-160` (a backlog item, not an `ESC-*` record) in its Deviations cell — non-blocking, corrected disposition guidance only, no verification-status impact.
...
> **Post-open CI-fix restatement requirement (LL-v8.5-P4-01, 2nd carry-forward, applied v8.6):** When a story's PR required a post-open CI-triggered fix (PR opened, CI failed, a fix commit was then pushed), the `Comments:` field must restate the specific final CI run confirming green — the run ID or "all N checks green," plus the `head_sha` actually tested — not just record that the fix was applied. "Fixed in a follow-up commit; re-run pending" alone does not satisfy this requirement once the re-run has actually completed. **Applies per CI-triggered fix, not once per EPIC (LL-v8.7-P4-01, applied v8.7):** when an EPIC contains multiple stories and more than one independently required a post-open CI-triggered fix, each such story's own `Comments:` field must carry its own restatement — satisfying the requirement for one story in the EPIC does not satisfy it for a sibling story's separate post-open fix.
