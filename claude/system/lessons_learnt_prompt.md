...
**Re-verify carried-forward claims against the current state, not the prior cycle's own record (LL-v9.1-Closure-01):** The two fixes above (`LL-v8.6-P4-01b`, `BLG-GOV-312`) both correct how a *deferred-patch-table* lookup is performed — by patch-ID, and by reading the named file directly. A related but distinct gap exists one level up: when this check restates a friction item's own "carried from `<prior_cycle_id>`, not yet due for escalation" status without independently re-running the patch-ID/named-file search against `prompt_change_log.md`'s **current** state (as it stands at the moment *this* cycle's check executes, not the prior cycle's own record of whether it was applied at *that* time), a patch that landed in the gap between the two cycles' filings goes undetected — the carried-forward line is copied forward as still-open when it has, in fact, already shipped. Confirmed live at `2026-09-03__release-v9.1`: its own Phase 4 section carried forward `execution_prompt.md`'s `test_scenarios` completeness check as "carried from v9.0, not yet applied, first carry" — but that exact patch (`LL-v9.0-P4-02`) had already shipped 4 days earlier, at `2026-08-21__release-v9.0`'s own post-ship closure (`execution_prompt.md` v3.70→v3.71, 2026-09-03). Applying `LL-v8.6-P4-01b`'s patch-ID search at the moment of *this* cycle's own check — not only when first evaluating the immediately-prior cycle's friction record — would have caught it. **Rule:** whenever this check states a deferred patch is "carried forward" from any prior cycle (not only the immediately-preceding one), re-run the patch-ID search (and, per `BLG-GOV-312`, read the named target file directly) against `prompt_change_log.md`'s state as of *this* run, before restating the prior cycle's own "not yet applied" conclusion as still current.
