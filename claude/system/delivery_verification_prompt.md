...
**Status:** Active
**Version:** 3.12
**Last Updated:** 2026-09-23 (post-ship closure 2026-09-21__release-v9.6 follow-up, Head of Specs Team ruling on `ESC-CLOSE-20260923-02` — §7 `LL-v9.1-P4-01` "or equivalent" evidence clause extended to open (not only resolved) deviations with no natural canonical-spec home); prior — 2026-09-14 (post-ship closure 2026-09-09__release-v9.3 STEP 8 immediate action, Head of Specs Team direct action — §2.1 gains a `Pass, escalation open` Result value with defined semantics, distinct from `Pass_with_deviation`, LL-v9.3-P4-02); prior — 2026-09-09 (post-ship closure 2026-09-07__release-v9.2 outstanding-actions resolution, Head of Specs Team direct action — §2.1 gains a `Pass_with_deviation` Result value with defined semantics, LL-v9.2-P4-01); prior history retained — see prior entries in version control.
...
**Extended to open deviations with no natural canonical-spec home (resolved 2026-09-23, `ESC-CLOSE-20260923-02`, Head of Specs Team ruling, added v3.12):** `LL-v9.1-P4-01` above is scoped to the Resolved-deviation carve-out (`LL-v8.6-P4-03`) specifically. The same "no natural canonical-spec home" shape also occurs on **open** P1–P3 deviations that still require documented PO/DoQ acceptance and backlog traceability per the severity table above — an open deviation whose `spec_references` point only at an ops audit doc, a code/test file, or similar is not exempt from those requirements, but it likewise has no Known Deviations section to carry the acceptance record. For this shape: the deviation's own `backlog.md` entry (carrying the Description, Priority, Owner, and any PO/DoQ acceptance rationale) together with the referenced ops/audit doc or code artefact and this report's own Deviation Register entry is sufficient "equivalent" evidence — a Known Deviations table entry is not required to exist in a non-spec artefact for this case either. This does not relax the requirement for any open deviation whose `spec_references` includes a genuine product/component spec — that spec's own Known Deviations entry remains the required evidence there. First found at `2026-09-21__release-v9.6` (`BLG-BE-127` P2, `BLG-BE-128` P3, `DEV-EPIC05-ST21-01`/`BLG-QA-190` P3 — an ops audit doc and code/test files respectively), where the verifying engine applied `LL-v9.1-P4-01`'s reasoning by interpretive extension rather than find it stated directly for the open case.
...
- `Result` must be `Pass`, `Pass with notes`, `Staging-deferred (per CLAUDE.md §2 / shared_standards.md §16.11)`, `Pass_with_deviation`, or `Pass, escalation open`.
...
- `Pass, escalation open` (added v3.11, LL-v9.3-P4-02): distinct from `Pass_with_deviation` — valid when the acceptance criterion's own literal wording is fully met (nothing narrowed or omitted), but a related, non-blocking escalation tracks a stronger evidentiary bar than the AC itself requires. The `Comments` field must name the open escalation ID. Treated as functionally equivalent to `Pass with notes` for verification-status purposes — not a `Fail`, and not a deviation (no backlog item is required in `Comments`, since nothing was narrowed or unmet; the escalation's own owning authority and SLA are the tracking mechanism). Formalises a value found in live use at `2026-09-09__release-v9.3` (`qa_evidence_EPIC-05.md`, ST-22/`ESC-EXEC-20260910-01`, originally recorded as the ad hoc "Pass, with open escalation" label) before this enumeration recognised it.
...
- `Pass with notes`, `Pass_with_deviation`, and `Pass, escalation open` results have substantive comments (not blank) — for `Pass_with_deviation`, the comment must name the specific AC gap and the confirmed backlog item tracking it; for `Pass, escalation open`, the comment must name the open escalation ID
