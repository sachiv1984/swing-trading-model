...
**Status:** Active
**Version:** 3.79
**Last Updated:** 2026-09-19 (post-ship closure `2026-09-15__release-v9.5` follow-up, Head of Specs Team + Product Owner direct action — BLG-GOV-337: §7 gains a narrow, plan-authorised write exception for `claude/roadmap/workforce_capacity.md`; BLG-GOV-335: §3.2.A Criterion 1's live-interaction bar now judged against the verification method actually used, not merely the AC's own wording); prior — 2026-09-18 (ST-33/BLG-GOV-318, sprint execution `2026-09-15__release-v9.5` — §7 Write Scope Restriction gains an Opportunistic In-File Fix Disclosure Threshold: commit-message disclosure is sufficient for a small, low-risk incidental fix confined to a file already open for the current story; otherwise it needs its own backlog entry per the existing out-of-scope-finding exception); prior — 2026-09-18 (ST-31/BLG-GOV-332 — the existing Resolving-commit deviation-closure discipline, step 10a, was scoped only to §3.1.A's own numbered steps; §3.1.B and §3.1.D now explicitly cross-reference it); prior history retained — see prior entries in version control.
...
**Pre-merge sign-off format lint (LL-v9.3-P4-01):** Before an EPIC's PR is opened, validate the `Signed off by:` line of its `qa_evidence_EPIC-xx.md` Standard/Autonomous-Class Sign-Off Block against `delivery_verification_prompt.md` §-1.3's recognised-format list: a literal named role (e.g. `Director of Quality`); `<Role> (autonomous class)`; the agent-mediated pattern `(agent-mediated, <Role> role — §5.3)`; or the infrastructure co-sign form above. If the line matches none of these (e.g. a bare engine/session name with no role qualifier) — correct it to a recognised format before the PR opens. This catches the mismatch at Sprint Execution time rather than surfacing it as a Tier 2 structural finding requiring an interactive counter-sign at the next Delivery Verification run (found live at `2026-09-09__release-v9.3`, `qa_evidence_EPIC-05.md`).
...
- `claude/roadmap/workforce_capacity.md` — **narrow, plan-authorised exception (BLG-GOV-337, v3.79; Head of Specs Team + Product Owner ruling, agent-mediated per §5.3, 2026-09-19).** Permitted only when the cycle's sealed `sprint_backlog.md` explicitly names this file in an ST item's acceptance criteria or sequencing note — the write is then authorised by a plan the Product Owner already signed off at the Sprint Planning seal, not inferred by the engine. Not permitted: any other `claude/roadmap/*` file (`current_roadmap.md`, `scored_initiatives.md`, `initiative_register.md`, etc.); any edit not traceable to a named ST item; any prioritisation, scope, or capacity-band decision (the file may be documented *into*, never used to *decide*). Every such write must cite the authorising `sprint_backlog.md` line in its commit message. Retroactively ratifies ST-37/ST-38 at `2026-09-15__release-v9.5` (both writes traced to the sealed plan's "Within-EPIC only: workforce_capacity.md" note); the ruling extends to no other roadmap file.
...
Must not modify: `claude/cycles/<cycle_id>/stage4_backlog_slice.md` (sealed), `claude/cycles/<cycle_id>/amendments/*/amended_backlog_slice.md` (sealed), `claude/cycles/<cycle_id>/sprint_backlog.md` (sealed), `claude/roadmap/*` (beyond the `workforce_capacity.md` exception above), `claude/backlog/backlog.md` (beyond the narrow new-item-addition exception above), `claude/strategy/strategy_rules.md`, any governance document outside this routine's scope.
...
> **PR-already-merged precondition (LL-v9.1-P3-01):** If this EPIC has already reached STEP 3.2.B (its PR is open), run `gh pr view <pr_number> --json state` immediately before this commit — not only at STEP 4's own merge flow. If `state = "MERGED"`, the branch is now inert: do not commit to it. Instead, apply the change directly to `main` (or queue it for the next EPIC branch/governance commit) and note the redirect in `execution_state.json`'s `process_notes` array. This is distinct from LL-v6.8-P3-02 (which covers the engine's *own* STEP 4 merge causing a stray commit) — this precondition catches a *human*-initiated merge racing a live follow-up commit the engine doesn't yet know has been overtaken, found twice in one cycle at `2026-09-03__release-v9.1` (EPIC-02 PR #1536, EPIC-05 PR #1539) and reconciled after the fact both times via the LL-v6.8-P3-01 orphaned-post-merge-commit safety net. Checking here prevents the stranded commit rather than merely recovering from it.
>
> **Never amend a pushed commit (LL-v9.1-P3-02, extended v3.74 LL-v9.2-P3-02):** Once a commit has been pushed to the remote `exec/<cycle_id>/EPIC-xx` branch, a follow-up fix must be a **new** commit — never `git commit --amend` a commit already on `origin`. If `--amend` is used by mistake, git's non-fast-forward push rejection is a safety net, not a green light: resolve with `git reset --soft <original-sha>` + a fresh commit, never a force-push. Found twice in immediate succession at `2026-09-03__release-v9.1` (EPIC-01's `8be831f5`, EPIC-02's `15db3691`) — both caught by push rejection before any public history was rewritten, but the mistake itself is preventable rather than merely recoverable. **A second, distinct trigger path (found `2026-09-07__release-v9.2`): a failed intermediate `git commit`** (e.g. rejected by the commit-msg hook) leaves no new commit to amend — reaching for `--amend` in that state instead rewrites the message of, and can fold unrelated staged changes into, the most recent commit already on `origin`, even though the amend command gives no warning that its target predates the failed attempt. This guardrail applies identically regardless of *why* `--amend` was reached for: after a failed `git commit` attempt, the correct recovery is to fix the failure (e.g. the commit message format) and issue a **fresh** `git commit`, never `--amend` — there is no scenario in this workflow where amending a commit already on `origin` is the correct next step. If it happens anyway: the same recovery applies — `git reset --soft <original-sha>` + a fresh, correctly-scoped commit, never a force-push. (Confirmed live at `2026-09-07__release-v9.2`, EPIC-02's `2ca7b178`: caught before push via `git log`/`git show --stat`, recovered cleanly with no origin history rewritten.)
>
> **No dedicated STEP 3.2.C needed (confirmed, post-ship closure `2026-09-03__release-v9.1` outstanding-actions follow-up):** LL-v9.1-P3-01's own filing left open whether the PR-already-merged precondition above needed a separate step (a new STEP 3.2.C) rather than living inside STEP 3.1.A, since STEP 3.1.A is nominally the "if `autonomous`" story-handling path. Checked directly: §3.1.C (`delegated_qa`) step 3 explicitly reads "Commit and push per 3.1.A steps 3–9" — so it already inherits this precondition by cross-reference, not by duplication. §3.1.B (`delegated_backend`/`delegated_frontend`) and §3.1.D (`delegated_decision`) do not have the *engine* committing to the EPIC branch directly at all — §3.1.B step 5 surfaces the branch and commit format to a human assignee who commits it themselves, and §3.1.D's unblock path re-classifies the item and resumes into whichever of §3.1.A/§3.1.C actually applies once unblocked. STEP 3.1.A step 3 is therefore already the single choke point for every commit the engine itself makes to an EPIC branch — a dedicated STEP 3.2.C would duplicate, not extend, that coverage. No further change made.
...
1. All stories in the EPIC have `delegation_class: autonomous`. **Verification-class sub-criterion (LL-v4.5-EX-01 — pre-planning sprint pattern):** Criterion 1 may be satisfied when all stories' VERIFICATION is by document inspection only (regardless of EXECUTION class), provided criteria 2–4 are also met. Applies when: the EPIC's primary deliverable is a governance or spec document; no observable UI behaviour, staging run, or live system interaction is required. Does not apply to EPICs with `delegated_backend` execution where the deliverable is a running system component.
   **Live-interaction bar applies to the method actually used (BLG-GOV-335, v3.79; Head of Specs Team ruling, agent-mediated per §5.3, 2026-09-19):** "no ... live system interaction is required" is judged against the verification method the story *actually used*, not merely whether its AC permits a documentation-only fallback. If verification involved live system interaction (queries against a staging/production database, a live API call, a deployed-environment check) — even where the AC's own wording offered a no-live-access alternative — Criterion 1 is unmet and the EPIC takes the standard Director of Quality sign-off block. The engine may not resolve a borderline eligibility call in its own favour: where it cannot tell, treat Criterion 1 as unmet. A story that wants to stay autonomous-eligible takes the AC's no-live-access fallback path and decides that *before* verifying, not after. Found live at `2026-09-15__release-v9.5` EPIC-04/ST-22 (`qa_evidence_EPIC-04.md` self-disclosed the ambiguity and resolved it in its own favour); that EPIC's autonomous-class sign-off should be treated as unconfirmed pending an actual Director of Quality's review.
...
> **Same-step self-verification read-back (LL-v9.2-P3-01, v3.74):** Immediately after the persist commit above, run `gh pr view <pr_number> --json state` and re-read `execution_state.json`'s own `pr_status`/`status` fields for this EPIC — confirm both now read `merged` before emitting the halt message. This mirrors the fix already applied to step 10a's `deviations_filed` write (LL-v9.0-P3-01) for the same underlying failure mode: a plain narrative instruction ("commit NOW") with no mechanical self-check is memory-dependent and can silently fail to fire. Found live at `2026-09-07__release-v9.2`: the mandatory persist commit for EPIC-04's merge was not made in the session that merged it — `execution_state.json` was left recording `pr_status: open` for an EPIC whose PR (#1599) had, in fact, already merged. The staleness was not caught by that session's own halt output; it was only caught at the *next* EPIC's session start, via the unrelated LL-v9.1-P3-01 PR-already-merged precondition check reading the same stale field for a different purpose — a downstream, differently-scoped check catching it instead of the primary control designed to prevent it. This read-back converts step 3a from memory-dependent to mechanically-checkable: if either field does not read `merged`/`pr_status: merged` after the read-back, do not proceed to the halt message — correct the write first.
