...
**Backend `database` stub-isolation pattern (LL-v9.4-P3-01, post-ship closure `2026-09-14__release-v9.4` STEP 8, same-cycle application):** Any pytest test file that needs to reimport `backend/database.py` for stub/mock isolation must use the isolated-copy pattern — `importlib.util.spec_from_file_location` to load a private module instance, combined with a scoped `patch.dict` for the duration of the test — never a bare module-level `sys.modules.pop("database", None); import database`. A bare pop-and-reimport at module level permanently overwrites the shared `sys.modules["database"]` stub for the rest of the pytest session; because pytest collects and runs test files in alphabetical order, this silently leaks a real (unstubbed) `database` module into any later-collected test file that also imports it, producing a live Postgres connection attempt in a test that never touched the offending file. `tests/test_position_audit_log.py` is the canonical example of the correct isolated-copy pattern — cite it when authoring a new test with this need. Found live at `2026-09-14__release-v9.4` (EPIC-05/ST-23): `test_ai_output_sampling_service.py`'s bare pop-and-reimport leaked into `test_alerts_service.py` (alphabetically later), caught only as a post-push CI failure; the same latent pattern was found in `test_trade_plan_audit_log.py` and filed as `BLG-QA-178` (not yet fixed, out of scope of the story that found it).
