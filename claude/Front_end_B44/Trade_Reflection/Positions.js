...
export default function Positions() {
  ...
  const { data: positions, isLoading } = useQuery({
    queryKey: ["positions"],
    queryFn: () => base44.entities.Position.list("-entry_date"),
  });
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: (_, variables) => {
      ...
      if (exitingPosition) {
        // Build the closed trade object for the reflection modal
        setReflectionTrade({ ...exitingPosition, ...variables.data });
              ...
              return (
                <TableRow key={position.id}>
                  ...
                  <TableCell className="text-slate-400">
                    {Number.isInteger(daysHeld)
                      ? <span className={cn("text-xs font-medium", daysHeld < 10 ? "text-amber-400" : "text-slate-400")}>Day {daysHeld + 1} of 10</span>
                      : "—"}
                  </TableCell>
                  <TableCell>
