...
function EditForm({ rule, onSave, onCancel }) {
  ...
  const showThresholdField = isCreateMode ? true : THRESHOLD_TYPES.includes(rule?.type);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch(`/alerts/rules/${rule.id}`, {
        method: "PATCH",
  ...
  return (
    <div className="px-6 py-5 bg-slate-800/40 border-t border-slate-700/50 space-y-4">
      ...
      {showThresholdField && rule?.type === "stop_loss_approach" && (
...
export default function AlertThresholdsSection() {
  ...
  const [openEditId, setOpenEditId] = useState(null); // rule id with open form
