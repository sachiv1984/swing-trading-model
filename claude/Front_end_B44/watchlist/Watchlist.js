...
function sortEntries(entries) {
  return [...entries].sort((a, b) => {
    const sigDiff = SIGNAL_ORDER[a.signal_status] - SIGNAL_ORDER[b.signal_status];
...
export default function Watchlist() {
  ...
  const [modal, setModal] = useState(null); // null | { mode: "add" } | { mode: "edit", entry } | { mode: "edit-confirm", entry }
  ...
  const openTrashConfirm = (entry) => setModal({ mode: "edit-confirm", entry });

  const sorted = entries; // already sorted on mutations
      ...
      <div className="rounded-2xl bg-gradient-to-br from-slate-900 to-slate-800 border border-slate-700/50 overflow-hidden">
        {loadError ? (
          <div className="flex flex-col items-center justify-center py-16 gap-4">
            <p className="text-rose-400 text-sm">Unable to load watchlist. Please refresh.</p>
            <Button
              variant="outline"
              size="sm"
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-slate-700/50">
                  {["Ticker", "Market", "Entry Signal", "Target Entry", "Stop (Initial)", "Stop (Current)", "Actions"].map((h) => (
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        <Button
                          variant="ghost"
                          size="icon"
                          onClick={() => openTrashConfirm(entry)}
...
function EmptyState({ onAdd }) {
  return (
    <div className="flex flex-col items-center justify-center py-20 text-center px-6">
      ...
      <p className="text-sm text-slate-400 mb-6">Add tickers you're monitoring for entry opportunities.</p>
      <Button
        onClick={onAdd}
        className="bg-gradient-to-r from-cyan-500 to-violet-500 hover:from-cyan-400 hover:to-violet-400 text-white border-0 shadow-lg shadow-violet-500/25"
      >
