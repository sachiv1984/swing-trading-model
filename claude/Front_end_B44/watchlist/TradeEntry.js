...
export default function TradeEntry() {
  ...
  const handleSubmit = () => {
    ...
    createMutation.mutate({
      ...
      stop_price: parseFloat(formData.stop_price) || (costs.initialStop > 0 ? costs.initialStop : null),
            ...
            <div className="space-y-2">
              ...
              {formData.atr_value && costs.initialStop > 0 && (
                <p className="text-xs text-slate-500">
                  Suggested stop: <span className="text-rose-400">{costs.currencySymbol}{costs.initialStop.toFixed(2)}</span>
                  {" "}({currentSettings.atr_multiplier_initial}x ATR)
                </p>
              )}
            </div>
            <div className="space-y-2">
