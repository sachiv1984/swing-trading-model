...
## Canonical Effort Band → Days Conversion Table (added 2026-09-09, LL-v9.2-P-01)

Release Planning and Sprint Planning have each independently inferred day-equivalents for the `**Effort:**` band letters (`XS`/`S`/`M`/`L`) used throughout `backlog.md`, from the parenthetical ranges scattered inconsistently across individual items' own `Effort` fields (some state "(~0.5 day)", others "(~2-3 days)", with no single cycle-level midpoint documented centrally). This table is the single canonical source going forward — Release Planning and Sprint Planning should read from here rather than re-deriving it each cycle.

| Band | Typical range cited in backlog items | Canonical midpoint (days) |
|------|---------------------------------------|---------------------------|
| XS | <1h – ~0.5 day | 0.15 |
| S | ~0.5 – 1 day | 0.5 |
| M | ~1–3 days (commonly "~2-3 days" or "~2d") | 2.5 |
| L | ~3–4 days | 3.5 |
...
**Precedence rule (added 2026-09-23, LL-v9.6-P-Closure-01):** When computing a cycle's total-days figure, an item's own explicit day range/midpoint (where its `**Effort:**` field states one, e.g. "M (~2d)") always takes precedence over this table's band-letter-only canonical midpoint (e.g. the generic "M" → 2.5d). The canonical midpoint applies only to items whose `**Effort:**` field gives the bare band letter with no more specific figure. This resolves the non-reproducibility Friction Item 3 (`lessons_learnt.md`, `2026-09-21__release-v9.6` Release Planning) found: v9.5's published 27.99d total matched neither a band-letter-only reading (26.60d) nor an explicit-range-midpoint reading (28.35d) of its own slice, because no single rule said which wins.
...
## Cross-Role Pairing Rotation Note (added 2026-09-18, ST-37/BLG-GOV-322)

**Purpose:** §7.1 (Skill-Silo Alert, per-cycle governance-vs-execution load) and §7.2 (Cross-Role Workload Balance, per-role story-ownership concentration) each currently rely on ad hoc pull-forward/rebalance candidate naming at the cycle they fire — there is no standing guidance for *which roles* release planning should favour next, informed by the accumulated pattern across recent readings. This is **advisory input only** — it does not gate scope selection, and release planning may override it with a documented rationale (the same standard already applied to any advisory in this document).

**Reading the historical pattern (as of the `2026-09-14__scheduled` rebalance, the most recent full reading of both metrics):**
- **§7.1 Skill-Silo Alert:** rolling-3-cycle average has worsened or stayed elevated for **4 consecutive readings** (`2026-08-11__scheduled` 89.8% → `2026-09-14__scheduled` 94.1%), consistently driven by GOV/SPEC/QA-process-shaped backlog composition rather than execution-heavy (BE/FE/OPS) items. The mandatory ≥2-build-and-ship-U-item pull-forward clause (`roadmap_prompt.md` §7.1) has fired 2 consecutive cycles without finding 2 qualifying candidates — the backlog itself is currently U-item-scarce, not a role-selection problem alone, but role rotation is the lever release planning *does* control.
- **§7.2 Cross-Role Workload Balance:** the most recent computation (v9.1–v9.3 window) found **Head of Specs Team** the highest-concentration role at ~24% of tallied stories — below the 40% ceiling (no advisory fired), but the clear leader among named roles, consistent with the Skill-Silo pattern above (Head of Specs Team is the most frequent owner of GOV/SPEC-shaped process and prompt-maintenance stories).

**Rotation guidance (advisory, not a hard rule):** When release planning selects scope under an active or recently-active §7.1 Skill-Silo Alert, favour candidate items owned by execution-heavy roles — **Backend Engineering Patterns Owner, Base44 Frontend Prompt Owner, Frontend Specifications & UX Documentation Owner, Infrastructure & Operations Owner** — over further Head of Specs Team-owned governance/process items, unless a genuinely time-sensitive governance gap (a hard gate, an audit finding, a compliance deadline) makes deferring it the worse choice. This does not mean rejecting Head of Specs Team-owned work outright — it means treating it as the pairing partner to rotate *away from* by default when a real execution-heavy candidate of comparable priority is also available, since it is both the current concentration leader (§7.2) and the primary driver of the sustained Skill-Silo pattern (§7.1).

**Maintenance:** Re-derive this note's specific role names and percentages whenever §7.2 is next recomputed (the "3 cycles since last computed" cadence already in use) — do not let it go stale against a role that has since rotated out of the concentration lead. This is a lightweight reference note, not a governance prompt; it does not require the `CLAUDE.md §6` version-bump checklist, matching the Canonical Effort Band table's own maintenance convention above.

**Cross-referenced from:** `roadmap_prompt.md` §7.1's mandatory pull-forward step — see that section for the mechanical trigger this note informs.
...
## Rebalance 2026-09-14__scheduled

**Tier:** Standard (0 active initiatives, CPS=N/A; not Extended — 34 days since `last_scheduled_rebalance_utc`, well under the 90-day threshold).

**FTE changes:** None — 0 active initiatives (12th consecutive scheduled cycle at this count).

**Idea intake:** `IW-20260914-01`, 44 submissions / 22 agents (register held 0 open ideas). 42 filed to `backlog.md` (all `Provisional-Target: TBD`, S/M effort); 2 Challenger submissions resolved as process patches via STEP 11.4 meta-review, not filed as backlog items.

**Skill-Silo Alert (STEP 7.1):** rolling-3-cycle avg **94.1%** (v9.1 87.8%, v9.2 94.6%, v9.3 100.0%) — >40% ceiling, **4th consecutive worsening/unresolved reading** since the mandatory clause first crossed threshold at `2026-08-11__scheduled` (89.8%→94.1%, following 65.8%→89.8%). Exhaustive gate-status scan of all 36 `BLG-FEAT-*`/`BLG-FE-*` items (36 checked programmatically, cross-verified against 2 known prose-gated items) found **0** ungated build-and-ship U-item candidates — down from 1 (`BLG-FEAT-32`, since shipped) at the prior reading. PO response: accept the shortfall; name this cycle's newly-filed `BLG-FEAT-95` ("trade plan required before entry" UI soft-nudge — ungated, genuine) as the sole qualifying candidate, escalated P3→P2; name `BLG-FEAT-73` (SI-02 frontend build) as a secondary candidate explicitly flagged `[gate status unverified/unmet]` per the LP-05 exception clause, since no 2nd genuine ungated candidate exists. Escalated to Head of Specs Team as a recurring pattern (2nd consecutive cycle where the full ≥2-item requirement could not be met even after a complete 44-submission idea-intake window) — see `lessons_learnt.md`.

**Cross-Role Workload Balance (STEP 7.2):** Recomputed this cycle (carried forward per the prior cycle's own outstanding action — 3rd cycle since last computed). Consolidated `**Owner:**` tally across `sprint_backlog.md` for v9.1/v9.2/v9.3: Head of Specs Team highest at ~24% (34 of 142 raw entries, consolidated-role-name basis) — below the 40% ceiling. **No advisory fires.** Data-quality caveat: owner-field naming is inconsistent across these 3 files (e.g. `QA Testing Owner`/`QA & Testing Owner`/`QA Lead` used interchangeably for what appears to be the same role) and raw entry counts do not fully reconcile against each changelog's total story count (likely reflecting some stories with 2 listed owners) — filed as a friction item in `lessons_learnt.md` rather than resolved inline this cycle.

**Product Value Ratio (STEP 2.4):** 0.092 🔴 Alert (window v8.9–v9.3, U=16/G=48/D=110/P=0/174) — 2nd consecutive Alert-tier reading, new low (down from 0.110 at `2026-08-11__scheduled`). PO response: accept; the root-cause structural fix (`BLG-BE-91`) already shipped v8.6 (2026-08-11) and has not yet had the time/trade-volume it needs to manifest in a fresh reading (per `BLG-FEAT-73`'s own PO-set re-check cadence, no earlier than 2026-11-09 or +10 new linked trade_plans — neither met). No further structural action taken this cycle beyond letting the existing fix mature.
