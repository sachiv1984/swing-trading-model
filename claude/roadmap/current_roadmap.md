...
**v7.6** — PDF / Print-Friendly Export — ✅ Complete — Shipped 2026-07-20 — cycle: 2026-07-20__release-v7.6
**Next planned release:** v7.7 — Strategy Intelligence Surfacing & Notification UX — anchor scope named DL-074 (see §3)
...
<!-- roadmap-annotation-marker: RA:v7.7:2026-07-21__formalization -->
...
|---|---|---|---|---|
| SI-02 Behavioural Drift Detection — frontend build | BLG-FEAT-73 | P1 | M (~2 days) | Arc 5 flagship "tell me when I'm deviating from my own rules" feature — backend has shipped since v4.6, no UI exists. `[gate status unverified/unmet — release planning to confirm before accepting into scope]`: BLG-GOV-107's 3 gate conditions last confirmed NOT MET at `2026-07-17__scheduled` (0/11 linked trade plans; drift `insufficient_data`, 9 trades in 90-day window) — not re-verified live this session. Named as forward intent per `roadmap_prompt.md` LP-05; may not enter sprint planning until independently reconfirmed met. |
| SI-04 Strategy Version Comparison | BLG-FEAT-75 | P2 | H (>5 days) | No blocking dependency — version-tagged trade history foundation already exists, unused. Ready. |
...
| Consolidate notification/digest surfaces | BLG-FE-114 | P2 | M (~1–2 days) | No gate. Concrete fix from the completed BLG-FE-112 consolidation audit — duplicate nav entries, orphaned Weekly Digest nav placement, no cross-link from digest alert-counts to the alert feed. Ready. |
| Confirm AiDailyBriefing light-theme rendering | BLG-FE-113 | P2 | XS–S | No gate. Same defect class as 3 prior shipped light-theme contrast bugs (BLG-FE-87/88/95). XS staging check, S fix if confirmed broken. Ready. |
