...
*No items currently named — Now horizon empty pending next `plan release` or roadmap rebalance scoping.*
...
> **SI-02 gate confirmation status (structured field — added 2026-07-08, LP-09; live re-confirmed 2026-07-12, roadmap rebalance 2026-07-12__scheduled):**
> **Last formally confirmed (total closed trades):** 20 (`GET /trades`, `total_trades: 20`, all 20 with non-null `pnl` — re-confirmed live 2026-07-12, unchanged since 2026-07-06)
> **Last formally confirmed (linked-plan gate condition, per the exact query above):** 0 — `GET /trade-plans` returns 11 trade-plan rows, **0** with non-null `position_id`. Re-confirmed live 2026-07-12. **Unchanged despite `BLG-BE-46`'s forward-fix shipping in v6.8 (2026-07-09)** — the fix auto-links new trade plans going forward but does not backfill; no trade plan has been created and closed since the fix shipped to populate a linked row. This is a data-timing gap, not a re-emergence of the original bug.
> **Condition (2) — p99 < 2s over 7-day window:** Not fully re-verifiable by a governed routine without historical latency monitoring data. Single live spot-check 2026-07-12: 1.107s response time — consistent with, but not proof of, the condition.
> **Condition (3) — drift scores confirmed meaningful:** **Explicitly re-confirmed FAILING**, live 2026-07-12 — `GET /analytics/behavioural-drift` returns `{"status": "insufficient_data", "analysis_window_days": 90, "trade_count_in_window": 9, "metrics": []}`.
