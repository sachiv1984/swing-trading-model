...
**v7.3** — Dashboard/Trade-Plan/Navigation UX continuation — ✅ Complete — Shipped 2026-07-16 — cycle: 2026-07-16__release-v7.3
**Next planned release:** [TBD] — `BLG-FE-115/116/117/118` unblocked (readiness passes complete, §13 pre-checks PASS where applicable) and ready for v7.4 scoping at next `plan release`
...
### v7.4 — UI Feature Expansion (Command Palette, Alerts, Bulk Actions, Saved Filters)

Formally version-labelled 2026-07-17 (roadmap rebalance `2026-07-17__scheduled`, STEP 8.1 Option (a), under the `roadmap_prompt.md` v9.2 STEP 8.1 amendment that closes `BLG-GOV-240`). Previously carried as an un-versioned "Unblocked carry-forward items" heading since post-ship closure `2026-07-16__release-v7.3` (STEP 11, `manage roadmap`) re-added these 4 items after `RA:v7.3`'s partial retirement. All 4 items' pre-implementation readiness passes (`BLG-SPEC-91/92/93/94`) shipped v7.3, clearing each item's blocking gate — including two §13 pre-checks, both PASS (RISK-03 for `BLG-FE-116`, RISK-04 for `BLG-FE-117`). Anchor scope confirmed via `IDEA-product-owner-20260717-01` (Promoted-Added, `2026-07-17__scheduled` STEP 5) and reinforced by the mandatory Skill-Silo pull-forward response to `IDEA-challenger-20260717-02` (Promoted-Added) — see `run_manifest.md` STEP 7.1.

| Feature | ID | Effort | Notes |
|---|---|---|---|
| Command palette | BLG-FE-115 | — | P1; readiness pass `BLG-SPEC-91` shipped 2026-07-16 (v7.3) — `cmdk` flagged missing from `package.json`; pre-flight dependency add + supporting UX/QA work bundled in `BLG-SPEC-95` |
| Custom price alerts | BLG-FE-116 | — | P1; readiness pass `BLG-SPEC-92` shipped 2026-07-16 (v7.3) — §13 pre-check PASSED (RISK-03 cleared) |
| Bulk actions | BLG-FE-117 | — | P1; readiness pass `BLG-SPEC-93` shipped 2026-07-16 (v7.3) — §13 pre-check PASSED (RISK-04 cleared); confirmation-modal/undo-window spec bundled in `BLG-SPEC-95` |
| Saved filters / calendar view | BLG-FE-118 | — | P1; readiness pass `BLG-SPEC-94` shipped 2026-07-16 (v7.3) — `react-day-picker` flagged missing from `package.json`; pre-flight dependency add + empty-state UX spec bundled in `BLG-SPEC-95` |
| v7.4 UI-heavy release readiness bundle | BLG-SPEC-95 | L (~5-7 days) | P1; consolidated cross-cutting pre-work for all 4 items above (dependencies, UX specs, design review, Playwright/analytics/regression-tag coverage) — filed `2026-07-17__scheduled` |
