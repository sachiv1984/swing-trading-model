...
### Unblocked carry-forward items (un-versioned — pending next `plan release`)

Re-added by post-ship closure `2026-07-16__release-v7.3` (STEP 11, `manage roadmap`) after `RA:v7.3`'s partial retirement. All 4 items' pre-implementation readiness passes (`BLG-SPEC-91/92/93/94`) shipped this cycle, clearing each item's blocking gate — including two §13 pre-checks, both PASS (RISK-03 for `BLG-FE-116`, RISK-04 for `BLG-FE-117`). This is the same structural gap as the v7.2→v7.3 transition (`BLG-GOV-240`, not yet resolved): `roadmap_prompt.md` STEP 8.1 cannot formally version-label this section because the Now horizon is non-empty, so these items remain un-versioned until the next `plan release` names them.

| Feature | ID | Effort | Notes |
|---|---|---|---|
| Command palette | BLG-FE-115 | — | P1; readiness pass `BLG-SPEC-91` shipped 2026-07-16 (v7.3) — `cmdk` flagged missing from `package.json` as an implementation-time blocker; ready for v7.4 scoping |
| Custom price alerts | BLG-FE-116 | — | P1; readiness pass `BLG-SPEC-92` shipped 2026-07-16 (v7.3) — §13 pre-check PASSED (RISK-03 cleared); ready for v7.4 scoping |
| Bulk actions | BLG-FE-117 | — | P1; readiness pass `BLG-SPEC-93` shipped 2026-07-16 (v7.3) — §13 pre-check PASSED (RISK-04 cleared); ready for v7.4 scoping |
| Saved filters / calendar view | BLG-FE-118 | — | P1; readiness pass `BLG-SPEC-94` shipped 2026-07-16 (v7.3) — `react-day-picker` flagged missing from `package.json`; ready for v7.4 scoping |

*RA:v7.3 retired — see roadmap_archive.md 2026-07-16 (post-ship closure 2026-07-16__release-v7.3; partial retirement, 3/7 items — 4 unblocked items re-added above).*
...
> **SI-02 gate confirmation status (structured field — added 2026-07-08, LP-09; live re-confirmed 2026-07-16, roadmap rebalance 2026-07-16__scheduled):**
> **Last formally confirmed (total closed trades):** 20 (`GET /trades`, `total_trades: 20`, all 20 with non-null `pnl` — re-confirmed live 2026-07-16, unchanged since 2026-07-06)
> **Last formally confirmed (linked-plan gate condition, per the exact query above):** 0 — `GET /trade-plans` returns 11 trade-plan rows, **0** with non-null `position_id`. Re-confirmed live 2026-07-16. **Unchanged despite `BLG-BE-46`'s forward-fix shipping in v6.8 (2026-07-09)** — the fix auto-links new trade plans going forward but does not backfill; no trade plan has been created and closed since the fix shipped to populate a linked row. This is a data-timing gap, not a re-emergence of the original bug. **`BLG-FE-109` (added ad-hoc, 2026-07-15, still unblocked and awaiting next release planning as of 2026-07-16) targets this exact gap from the UX side** — a "Start Trade from Plan" action to make linkage the path of least resistance rather than a manual step nobody takes.
> **Condition (2) — p99 < 2s over 7-day window:** Not fully re-verifiable by a governed routine without historical latency monitoring data. Single live spot-check 2026-07-16: 1.149s response time — consistent with, but not proof of, the condition.
> **Condition (3) — drift scores confirmed meaningful:** **Explicitly re-confirmed FAILING**, live 2026-07-16 — `GET /analytics/behavioural-drift` returns `{"status": "insufficient_data", "analysis_window_days": 90, "trade_count_in_window": 9, "metrics": []}` (byte-identical to 2026-07-12/07-13/07-14/07-15 checks — 5th consecutive identical reading).
