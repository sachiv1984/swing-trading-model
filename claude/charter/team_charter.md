...
**Scheduled rebalance cadence guideline (BLG-GOV-208):** `run roadmap --reason "scheduled"` should not be invoked twice on the same calendar date absent explicit cause (e.g. correcting a run that failed partway, or a Product Owner-directed re-run following a same-day scope change). By this cycle, 55+ governed cycles had completed at sustained high governance intensity, including at least one same-day double-run (2026-07-10, resolved ad hoc — see `BLG-GOV-207`/`BLG-GOV-216`, both shipped v7.10). Those two items gave the roadmap engine itself a technical safety net (STEP -1 recency advisory; auto-suffixed `cycle_id` on collision), but neither constitutes a *policy* position on whether a same-day double-run should happen in the first place — this guideline is that position, not a third technical control. A same-day double scheduled-rebalance run is a workload/fatigue signal for the one human operator in this solo-plus-AI-delegation model, distinct from and complementary to the Skill-Silo Alert's product-mix framing (`BLG-GOV-209`). Absent explicit cause, a second same-day scheduled-rebalance run should be deferred to the next calendar day; if explicit cause exists, it should be recorded inline in that cycle's `decision_log.md` entry.
...
8. **No same-day double scheduled-rebalance run absent explicit cause.** `run roadmap --reason "scheduled"` should not be re-invoked on the same calendar date without a recorded cause (see Director of HR's scheduled rebalance cadence guideline, §3.1, BLG-GOV-208). Complements the roadmap engine's own STEP -1 recency advisory and `cycle_id` auto-suffix (`BLG-GOV-216`, `BLG-GOV-207`) with an explicit policy position, not merely a technical safeguard.
