...
**Last Updated:** 2026-08-16 (session — 1 new item added: BLG-OPS-146); prior — 2026-08-16 (session — 2 new items added: BLG-QA-151, BLG-BE-101); prior — 2026-08-16 (session — 1 new item added: BLG-QA-150); prior history retained — see prior entries in version control.
...
### BLG-OPS-146 — Pin local dev Python version to match CI/staging; confirm PUBLIC_URL parity on production
...
**Problem:** The ST-20 test-environment parity check (`docs/ops/test_environment_parity_check_2026-08-16.md`) found local dev running Python 3.14.4 against `backend/.venv` while CI (all 4 workflows, 7 occurrences) and staging (`render.yaml`) both pin 3.11 — no `.python-version` file or setup doc anywhere in the repo specifies an intended local version, so this can drift silently per-machine. Separately, `.env.staging` and `render.yaml`'s staging block both set `PUBLIC_URL=/` (overrides `package.json`'s `homepage` so asset paths resolve at root); `.env.production`'s repo template omits it — likely already set correctly in the Render dashboard (production env vars are dashboard-only, not repo-visible) but unconfirmed.
**Scope:** (a) Add a `.python-version` file (or equivalent pin/doc) targeting 3.11 for local backend dev; (b) confirm via Render dashboard whether production has `PUBLIC_URL` set, and either add it to `.env.production`'s template for documentation parity or note why it's not needed there.
**Acceptance Criteria:** `.python-version` (or equivalent) added and confirmed to resolve to 3.11 in a fresh local venv setup; production `PUBLIC_URL` status confirmed and documented; Infrastructure & Operations Owner sign-off.
