...
| 2026-06-16 | sprint-planning | STEP -1.7 prompt change log gap check used `grep | tail -N` to find the "last logged version" in `prompt_change_log.md`. Since the file is prepended newest-first, `tail` returns the oldest entries — the most recent entry was at the top but unseen. OA-SP-01/02 in v5.7 sprint planning were both false positives (entries existed at lines 25 and 123). | Use `grep "<filename>" claude/system/prompt_change_log.md \| head -1` to find the most recent logged transition. Never use `tail` on a prepend-sorted file to find the latest entry. Also check per-file changelogs (changelogs/*.md) — these are separate from the global log and may lag if not explicitly updated post-refactor. |
...
| 2026-09-08 | backlog-add | Filing BLG-SPEC-137: the skill's namespace table and the 2026-04-03 lesson above both assume `backlog.md` still has 8 numbered type sections (§1–§8) and a `BLG-SPEC-Dxx` (padded) namespace. Neither is true today — `grep -n "^## "` on the live file shows only 3 numbered headings (§1 Platform, §2 Product Feature, §3 Frontend & UX); everything from §3 onward (Backend, QA, Ops, Spec Debt, Governance items alike) has been flat-appended end-of-file for a long time with no type heading at all, and the live spec-debt namespace is bare `BLG-SPEC-xx` (confirmed up to `BLG-SPEC-136`), not `BLG-SPEC-Dxx`. | Before filing: (1) `grep -n "^## " claude/backlog/backlog.md` to see what section headings genuinely exist right now — don't trust the skill doc or a prior lesson's description of the structure. If the target type section doesn't exist as a heading, don't invent one; append at end-of-file (`tail -n 40` to find the actual last item, insert after its closing `---`), matching current real practice. (2) For ID namespace, `grep -oE "BLG-SPEC-D?[0-9]+"` (or the equivalent pattern for the namespace in question) across both `backlog.md` and `backlog_archive.md` to find the real live convention before assuming the skill table's example format. |
