...
- **When adding new `database` imports to `backend/services/position_service.py`, update the stub function lists in all four database-stub test files in the same commit:** `tests/test_alerts_service.py`, `tests/test_watchlist_service.py`, `tests/test_trade_service.py`, and `tests/test_service_coverage.py`. These files inject a `types.ModuleType("database")` stub into `sys.modules` at collection time; any function missing from the stub causes an `ImportError: cannot import name '...' from 'database' (unknown location)` in CI even though the function exists in `database.py`.
