...
2a. **Identical-text-masks-differing-semantics check (mandatory, before resolving any conflict below).** For any shared *governance prompt or Class 6 document* touched by both sides (not just the files explicitly named in step 3), check whether both branches independently bumped the same `**Version:**` header field to the same literal version number for two *different* changes. `git merge` sees no conflict when the version-line text is byte-identical on both sides — the collision is silent, and one branch's change can end up applied under a version number that no longer uniquely identifies it. Before accepting either side's version number as-is: confirm the two branches' *change descriptions* (their own `Change Log` entries or `prompt_change_log.md` rows) are actually the same change, not two coincidentally-matching version bumps. If they are different changes: renumber the later-merging branch's bump to the next free version (and its `Change Log`/`prompt_change_log.md`/`OPERATIONAL_GUIDE.md` §14 rows) *before* committing the merge resolution — do not let two different changes ship under one version number. Record any renumbering performed in the conflict-resolution commit message. (Real precedent for the numeric half of this pattern, `sprint_planning_prompt.md` v3.13/v3.14 both independently cut, is in `prompt_change_log.md`'s `2026-08-03__release-v8.1` entries — this check exists to make that renumbering step deliberate rather than incidental.) **Check every co-bumped file individually — a caught collision on one file does not clear its siblings (2026-08-17, `2026-08-14__release-v8.8` Phase 3 lessons learnt):** when a single story bumps `OPERATIONAL_GUIDE.md`'s version *and* the source-prompt file it's citing (the normal §6 pattern — e.g. `post_ship_closure.md`) in the same commit, a genuine collision on one of the two does not imply the other is clear, or vice versa. At `2026-08-14__release-v8.8`, `EPIC-07`'s ST-29 independently collided with `EPIC-03`'s ST-13 on *both* `OPERATIONAL_GUIDE.md` v4.162 and `post_ship_closure.md` v2.27 — the first was caught and disclosed at commit time, but the second went completely unnoticed until a PR review ran an actual test merge, because the check had only been applied to the first file found, not repeated against every file the same commit bumped. Apply this check per file, not once per commit.
2b. **Shared-JSON-field schema-shape drift check (mandatory, before resolving any conflict below — ST-18, BLG-GOV-290, v8.7).** For any shared JSON file touched by both sides (`execution_state.json` and any other shared JSON structure named in step 3, or introduced later), check whether both branches independently added or modified the *same field name* with a *different shape* (e.g. one branch adds `"retry_count": 3` as a bare integer, the other adds `"retry_count": {"count": 3, "last_attempt": "..."}"` as an object) or a *different meaning* under the same name/type (e.g. two branches both add a boolean `"gated"` field to different-but-structurally-similar records, one meaning "feature is gate-blocked" and the other meaning "user has not yet unlocked"). Unlike the version-number collision in 2a, `git merge` frequently sees **no conflict at all** here even when the two additions are genuinely incompatible — if the two branches' edits land on non-overlapping lines of the JSON file (common when each branch's story touches a different key or a different array element), the line-based merge succeeds cleanly and the resulting file silently carries two incompatible shapes for what looks like one field. Before accepting a clean auto-merge on any shared JSON file as-is: diff the *set of field names* each branch touched against the *other* branch's diff on the same file; for any name in common, confirm both sides used the same shape/type/meaning, not just that the merge produced valid JSON. If they conflict semantically: resolve to one agreed shape (favouring whichever branch's shape is used by more existing readers, or escalate to the relevant spec owner if usage is even) *before* committing the merge resolution, and record the resolution in the conflict-resolution commit message. This check has no version-line text to compare (schema shape isn't announced anywhere `git diff` surfaces on its own) — it requires deliberately re-reading both branches' actual field-level changes, not just trusting a clean merge.
