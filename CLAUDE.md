...
- **When adding new `database` imports to `backend/services/position_service.py`, update the `_DB_STUB_FUNCTIONS` list in `tests/conftest.py` in the same commit.** This file registers a session-scoped `types.ModuleType("database")` stub in `sys.modules` at collection time (BLG-QA-20); any function missing from the stub causes an `ImportError: cannot import name '...' from 'database' (unknown location)` in CI even though the function exists in `database.py`.
