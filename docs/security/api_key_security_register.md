...
**Cycle:** 2026-08-05__release-v8.3 (ST-04 — BLG-SEC-17: entry #3 Anthropic/"Gemini" API key rotation runbook added; ST-03 — BLG-OPS-131: entry #6 updated with new `STAGING_API_KEY` GitHub Actions secret requirement)
...
| Storage location | Render environment variable `API_KEY` on each service independently (production `srv-d5r98jm3jp1c73figm1g`; staging backend `srv-d6rtdg94tr6s73ce2j6g`; staging frontend `srv-d6rtdg94tr6s73ce2j60` as both `API_KEY` and `REACT_APP_API_KEY`); GitHub Actions repo secrets `API_KEY` and `REACT_APP_API_KEY` carry the production value (consumed by `alert-evaluation.yml`, `daily-snapshot.yml`, `backtest.yml`, `deploy.yml`); local copy of the production value in `~/.api_keys` as `RENDER_API_KEY` for use by governed-routine sessions. **New secret required (ST-03, BLG-OPS-131, EPIC-01, v8.3):** `.github/workflows/api-key-cross-environment-check.yml` additionally needs the *staging* value as a GitHub Actions repo secret named `STAGING_API_KEY` — **human action required**: Infrastructure & Operations Owner to add this secret (`gh secret set STAGING_API_KEY`) from the staging backend service's current `API_KEY` value. The workflow no-ops gracefully with a `::warning::` until this secret is added. |
