...
**Follow-up implementation item (recommended, not filed by this gate — outside its write scope, see `design_gate_prompt.md` §5):** convert the 4 hardcoding consumers (`WatchlistModal.js`, `ExportModal.js`, `PositionEntryModal.js`, `WidgetLibrary.js`) from `bg-slate-900`/`text-white` to the shared `bg-background`/`text-foreground`/`text-popover`/`text-popover-foreground` token set — a genuinely new implementation item (this design decision alone does not ship the fix); PMO Lead/Product Owner to file to the backlog. Sequencing note for that filing: this follow-up should land no earlier than `ST-04`/`BLG-FE-147` (this cycle), since `bg-popover`/`text-popover-foreground` are among the tokens `ST-04` registers in `tailwind.config.js` — building the fix before those tokens compile would silently reproduce the exact "empty CSS rule" failure mode `BLG-FE-147` exists to close.
