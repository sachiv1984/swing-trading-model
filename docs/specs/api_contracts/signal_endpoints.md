...
| `allocation_gbp` | Estimated GBP cost of the risk-based position size (from `sizing_service.size_position()` — includes estimated fees). Equals `estimated_cost` from the canonical sizing calculator |
