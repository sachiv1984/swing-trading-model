...
**Status:** Canonical
**Version:** 1.22.0
**Last Updated:** 2026-09-22 (ST-08, EPIC-02, v9.6, BLG-FR-05 — added "Month-end immutability and restatement" paragraph under Fee-Netting Basis: documents the new `monthly_pnl_snapshots` baseline-once mechanism and discloses it is currently unreachable in production, no live write path can trigger a restatement); prior — 2026-09-22 (ST-07, EPIC-02, v9.6, BLG-FR-04 — new "Fee-Netting Basis" subsection under Realized/Unrealized P&L Split: documents `realised_pnl_gbp` is net of both entry and exit fees, and flags that a NULL `entry_fees`/`exit_fees` leg is silently treated as zero rather than unknown); prior — 2026-09-15 (ST-15, EPIC-04, v9.4, BLG-SPEC-138 — Appendix F gains an explicit placement-decision cross-reference; content unchanged, no relocation); prior history retained — see prior entries in version control
