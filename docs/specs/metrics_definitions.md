...
**Status:** Canonical
**Version:** 1.23.0
**Last Updated:** 2026-09-23 (ST-26, EPIC-06, v9.6, BLG-SPEC-146 — Sharpe Ratio section gains a canonical Lookback Window subsection, with the discrepancy against each of the 3 current call sites documented); prior — 2026-09-22 (ST-08, EPIC-02, v9.6, BLG-FR-05 — added "Month-end immutability and restatement" paragraph under Fee-Netting Basis: documents the new `monthly_pnl_snapshots` baseline-once mechanism and discloses it is currently unreachable in production, no live write path can trigger a restatement); prior — 2026-09-22 (ST-07, EPIC-02, v9.6, BLG-FR-04 — new "Fee-Netting Basis" subsection under Realized/Unrealized P&L Split: documents `realised_pnl_gbp` is net of both entry and exit fees, and flags that a NULL `entry_fees`/`exit_fees` leg is silently treated as zero rather than unknown); prior history retained — see prior entries in version control
