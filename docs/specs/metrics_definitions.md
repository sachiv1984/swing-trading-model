...
### Lookback Window (CANONICAL, ST-26, EPIC-06, v9.6, BLG-SPEC-146)

Neither the Method Selection nor Data Requirements sections above bound *how far back* a method looks — only the minimum sample size (30+ snapshots / 10+ trades). Left unstated, three independent Sharpe implementations in this codebase each picked a different answer. This section fixes that gap.

**Canonical window: trailing 252 trading days** (≈1 calendar year), applied as an upper bound on top of the existing method-selection priority — i.e. "portfolio method, ≥30 snapshots, within the trailing 252 days; else trade method, ≥10 trades, within the trailing 252 days; else insufficient_data." A user with fewer than 252 days of history simply uses everything available (no change from today for a new account); the window only starts trimming once more than a year of history has accumulated.

**Rationale:** (1) every implementation already annualises by `× √252`, so a 252-trading-day window makes the "annualised" framing literal rather than notional — a live reading that only ever asked "how has this year gone" is annualising a period it actually measured, not a mix of "however much history happens to exist" scaled up to a year. (2) This is a *live* risk-adjusted-return reading a user checks on an ongoing basis, not a fixed historical record — as an account accumulates multiple years of history, an unbounded/all-time window increasingly averages a stale early regime (different market conditions, possibly a different or less-tuned strategy version) into a number meant to answer "how is this doing right now." A rolling window keeps the reading current. (3) `production_strategy.py`, the one existing call site that already imposes an explicit fixed window, independently arrived at the same 252-day figure for its own backtest Sharpe — this canonicalises the convention already in de facto use there, rather than inventing a fourth number.

**Discrepancy against each current call site** (documentation only in this story — fixing any one of these is separate follow-on work):

1. **`backend/services/analytics_service.py::_build_executive_metrics`** (the canonical API field, `executive_metrics.sharpe_ratio`, served to the dashboard/analytics UI) — computes over `filtered_history`, i.e. whatever the caller's `period` filter selects (defaults to `all_time`, effectively unbounded). No 252-day cap is applied even when far more than a year of snapshots exists. This is the primary implementation the new canonical window is meant to constrain.
2. **`production_strategy.py`** (offline backtesting/research script, not a live API or UI call site) — already uses a fixed 252-trading-day `lookback` parameter for its own Sharpe (`cagr / vol`, both computed over that window). Conforms to the canonical window by coincidence, not by reading this spec; also uses a different formula shape (CAGR ÷ annualised vol, rather than mean(daily returns) ÷ stddev(daily returns) × √252) — a second, narrower discrepancy from the Canonical Formula above, noted here for completeness but out of this story's scope (window only).
3. **`src/pages/PerformanceAnalytics.js::calculateMetrics`** — an independent, frontend-only Sharpe calculation (does not consume the API's `executive_metrics.sharpe_ratio` field at all). Window is whatever the page's own period selector currently has set (`last_3_months` / `last_year` / `ytd` / unbounded default), not the canonical 252-day figure under any of those settings except by coincidence with `last_year`. Also diverges from the Canonical Formula in two further ways, noted here for completeness: it computes over per-trade `pnl_percent` values rather than daily portfolio-value returns (closer in shape to the spec's Trade-Based method, but without that method's 10+ trade minimum-sample check), and it divides by `returns.length` (population variance) rather than `returns.length − 1` (sample variance), contradicting the Variance Convention above.
...
### Fee-Netting Basis (ST-07, EPIC-02, v9.6, BLG-FR-04)

**Finding: `realised_pnl_gbp` is net of fees, on both legs.** `trade_history.pnl = net_proceeds − total_cost`, where `total_cost` includes entry commission/stamp duty (`total_cost_gbp`, derived from `positions.fees_paid` at entry — see `position_service.py`'s `entry_fees` computation) and `net_proceeds` is gross exit proceeds less exit commission (`exit_fees`). No further fee adjustment is applied at report time. This confirms `reports_endpoints.md`'s existing "Fee treatment" derivation note (`GET /reports/tax-year`) applies identically to `GET /reports/monthly-pnl` and `GET /reports/daily-pnl`, since all three sum the same stored `pnl` column.

**Audit finding: a NULL fee leg is silently treated as zero, not as unknown.** `trade_history.entry_fees`/`exit_fees` are nullable columns (`data_model.md` §3). If either is `NULL` for a given closed trade — for example because the source `positions.fees_paid` was itself `NULL` at entry (a live schema drift already tracked at `BLG-SPEC-151`) — the trade's `total_cost`/`net_proceeds`/`pnl` were computed with that fee leg contributing nothing, rather than being flagged as fee-basis-incomplete. This means such a trade's stored `pnl` may overstate realised profit (or understate a loss) by the amount of the missing fee. **Disposition (RISK-02):** this is an audit-and-flag finding, not a figure correction — no stored `pnl`/`total_cost`/`net_proceeds` value is recomputed or changed by this story. A trade found to have a genuine gross-vs-net discrepancy is tracked as its own fix item, not corrected retroactively inside a reporting change.

**Flag, not fix:** `GET /reports/monthly-pnl`'s per-month response gains `null_fee_trade_count` (v0.13) — the count of that month's closed trades where `entry_fees IS NULL OR exit_fees IS NULL` (`backend/database.py::get_monthly_pnl()`). A non-zero count is visible in the Monthly P&L table as the audit signal; it does not gate or alter the displayed `realised_pnl_gbp` total. See `reports_endpoints.md` §GET /reports/monthly-pnl for the full field definition.

**Month-end immutability and restatement (ST-08, EPIC-02, v9.6, BLG-FR-05):** every closed calendar month's `realised_pnl_gbp`/`trade_count` is baselined exactly once into `monthly_pnl_snapshots` (`data_model.md` DS-20) the first time it is read as closed, and never overwritten thereafter. `GET /reports/monthly-pnl` and `GET /reports/tax-year` both compare the live-computed figure against that baseline on every read and flag a mismatch (`restated` / `restated_month_count`). As of this story, no live write path exists that could actually change a closed trade's `pnl`/`exit_date` after creation, so a restatement cannot currently occur in production — the mechanism is real and tested, but its trigger condition is presently unreachable (see DS-20 for the full disclosure). This is distinct from `fee_drag`/cost-basis disclosures above, which describe how a figure was computed; the snapshot mechanism instead detects whether a previously-reported figure has since changed.
...
**Definition:** Governance Overhead Ratio = (sum of elapsed wall-clock time for governance-classified routine sessions) ÷ (sum of elapsed wall-clock time for **all** routine sessions), over a trailing quarter (90 days). "Governance-classified" routines are Roadmap Rebalance, Release Planning, Design Gate, Sprint Planning, Delivery Verification, and Post-Ship Closure — process/coordination sessions that do not themselves write shipped product code. "Shipped-side" routines are Sprint Execution sessions (the only routine whose own work directly produces merged product commits). This mirrors the Skill-Category Taxonomy's governance-heavy/execution-heavy split above, applied to session time rather than role headcount.

**First baseline reading (2026-09-24, this story):** **Not yet meaningfully computable.** §22's logging convention is applied from `2026-08-03__release-v8.1` onward, but has only actually been wired into an engine's own STEP list for `roadmap_prompt.md` (per `OPERATIONAL_GUIDE.md` changelog row 4.192) — `execution_prompt.md`, `sprint_planning_prompt.md`, `delivery_verification_prompt.md`, `post_ship_closure.md`, and `design_gate_prompt.md` do not yet capture `Session start`/`Session end` in their own written records, and `release_planning_prompt.md`'s own manifests happen to carry the fields without a corresponding `§22` citation in that prompt's own text (an undocumented-but-present case, not separately investigated here — out of this story's S-effort scope). Real recorded readings found (all governance-side; grepped across `claude/cycles/**/run_manifest.md`):

| Cycle | Routine | Elapsed |
|-------|---------|---------|
| `2026-09-19__scheduled` | Roadmap Rebalance | 36m01s |
| `2026-09-21__release-v9.6` | Release Planning | 9m00s |
| `2026-09-23__release-v9.7` | Release Planning | 1h08m41s |

**Governance-side total: 1h53m42s. Shipped-side (Sprint Execution) total: 0 (no session has recorded a Session start/end pair yet).** A ratio computed today (governance ÷ total) would trivially read 100% for lack of any shipped-side denominator data — not a real reading of governance overhead, just an artefact of incomplete instrumentation. The metric is correctly defined and ready to compute a first genuine reading once at least one Sprint Execution session's own wall-clock time is captured; recording that gap explicitly here (rather than a fabricated or extrapolated ratio) satisfies this story's own "first baseline reading recorded" AC.

**Sign-off:** Metrics Definitions & Analytics Canonical Owner — Approved. Defining the metric and honestly reporting the current data-availability gap, rather than computing a ratio from an incomplete/one-sided denominator, is the correct disposition — a fabricated first reading would be worse than none. Sprint Execution Engine (agent-mediated, Metrics Definitions & Analytics Canonical Owner role — §5.3), 2026-09-24.
