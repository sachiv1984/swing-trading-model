...
Panel is hidden entirely (no placeholder) until **Stop Level** (§5.1, `planned_stop_price`) has a valid positive value — see `DEV-v8.9-ST05-01` in Known Deviations for why this is Stop Level alone rather than "both Planned Entry Price and Stop Level" (Planned Entry Price is itself an input inside this panel, so gating the panel's own visibility on it would make it unreachable). Once visible, the panel shows an empty **Planned Entry Price** input (new, panel-local field — see §5d.2); the calculation/output section (§5d.3) renders only once both Planned Entry Price and Stop Level are valid.
...
| R at Risk | Derived: `(plannedEntryPrice − stopLevel) × suggested_shares`, then GBP-converted for US-market plans via the response's `fx_rate_used` — same convention as `TradeEntry.js`'s own "Total Risk" row (`costs.totalRisk`, `totalRiskNative / fx_rate` for US) | GBP, `£X.XX` — always GBP-basis (not native currency; corrects the original design-gate wording, which said "Native currency" but the only shipped precedent for this exact figure is GBP-basis) |
| Portfolio Heat Impact | `heat_impact_percent` (confirmed at implementation, ST-05 — reuses `GET /portfolio/prospective-heat`'s calculation via `services/portfolio_service.py::calculate_prospective_heat`; `null` renders as "—") | "+X.X% heat" |
...
| DEV-v8.9-ST05-02 | ST-05 (EPIC-02, v8.9, BLG-FEAT-91): §5d.3's original "R at Risk... FX-converted for US market same as §10.7" was implemented literally as a raw `(entry − stop) × shares` with **no** FX conversion applied for any market — caught by Frontend Specifications & UX Documentation Owner sign-off review (2026-08-18), same class of currency-basis bug already fixed once this cycle (ST-02, BLG-BE-103, commit `275b6e4f`). Also, "Native currency" was itself imprecise: the cited precedent (`TradeEntry.js`'s "Total Risk" row, `costs.totalRisk`) is always GBP-basis, not native-currency. Fixed same-day, same story: `WhatIfSizingPreview.js` now reads `sizingResult.fx_rate_used` (always returned by `POST /portfolio/size`) and divides by it for US-market plans, matching `TradeEntry.js`'s exact convention; `£` symbol added; §5d.3's Format column corrected to "GBP" throughout. | §5d.3: R at Risk FX-converted for US market, native currency | P2 | Resolved — same commit (ST-05) | Frontend Specifications & UX Documentation Owner | N/A |
| DEV-v8.9-ST05-01 | ST-05 (EPIC-02, v8.9, BLG-FEAT-91): §5d.1's literal presence-gate wording ("hidden entirely until both Planned Entry Price and Stop Level have valid positive values") is self-contradictory as written — Planned Entry Price is itself an input *inside* this panel (§5d.2), so a panel hidden until its own field is filled could never be filled through normal use. Implemented gating on Stop Level (`planned_stop_price`, an existing external form field) alone: once a valid stop is set, the panel appears with an empty Planned Entry Price input; the calculation/output section still renders only once both inputs are valid, preserving the spec's evident intent (no result shown from one input alone) without the unsatisfiable literal condition. | §5d.1: hidden until both Planned Entry Price and Stop Level valid | P3 | Resolved — implemented per corrected reading, same commit (ST-05) | Frontend Specifications & UX Documentation Owner | N/A |
| DEV-v8.6-ST02-01 | ST-02 (EPIC-01, v8.6, BLG-FEAT-56): §10.5's Setup Thesis Digest panel ships without the spec'd violet "AI draft" badge. `isAiDraft` is ephemeral client-only form state in `TradePlan.js` (`useState`, never persisted to `trade_plans`) — there is no server-side field for `TradeEntry.js` to read the source plan's AI-origin from at order-placement time. The digest still renders (thesis + risk factors + link), just without the badge, per ux_spec.md §4's own "panel still renders, simply without the badge" edge-case note. **Disposition (agent-mediated, on behalf of Product Owner, 2026-08-11 — per explicit user direction; ratified as the standing decision 2026-08-12, see `qa_evidence_EPIC-01.md`'s Ratification note):** Accepted as shippable. ST-02's actual AC ("digest renders using the existing Claude thesis generation service") is fully met; the badge is a design-spec visual cue one level below the committed AC, cosmetic-only (digest content is identical either way), and its root cause (isAiDraft never persisted) is a pre-existing gap properly scoped out to BLG-BE-95 rather than expanding this story. **RESOLVED (v8.7, ST-03, BLG-BE-95):** `is_ai_draft` column persisted on `trade_plans` (mirrors `isClaudeDraft` — real "Improve with AI" generation only); `SetupThesisDigestPanel` now reads it and shows the badge when true (AC-03, `qa_evidence_EPIC-01.md`, Pass) — root cause closed, badge reachable. | §10.5 (ux_spec.md §2): badge shown when source plan's isAiDraft was true at generation time | P3 | Resolved — v8.7 (ST-03, BLG-BE-95) | Head of Engineering | BLG-BE-95 |
