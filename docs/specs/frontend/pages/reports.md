...
> **v0.19 correction:** the v9.6 record specified `fees_missing_count`/`fees_missing_total`. The live API (`reports_endpoints.md` v0.13) instead returns `null_fee_trade_count` per month, with **no top-level aggregate field**. The field names and aggregate-derivation note below are corrected to match; the UX itself (notice/indicator/caption wording and placement) is unchanged from v0.18.

- **Aggregate notice** — directly under the "Monthly Realised P&L" heading, `StandingAlert` Info tone, non-dismissible, shown **only when** `N ≥ 1`, where `N` is the **sum of `null_fee_trade_count` across all months currently loaded in the table** (client-side derived, same convention as the Avg P&L/Trade column above — not a new backend field): `"{N} closed trade(s) have no fees recorded, so these figures may not reflect their costs."` (`data-testid="monthly-fees-missing-notice"`). Hidden at N = 0, while loading and on error.
- **Per-month indicator** — in the **Trades** cell, when a month's `null_fee_trade_count = k ≥ 1`: `"{trade_count} · {k} no fees"` (muted secondary text; `aria-label` `"{trade_count} trades, {k} without fees recorded"`; `data-testid="monthly-fees-missing-count"`). Months with `k = 0` render unchanged.
...
- **API:** `GET /reports/monthly-pnl` returns `null_fee_trade_count` per month (`reports_endpoints.md` v0.13). No top-level aggregate field exists or is added — the aggregate notice's count is derived client-side per the bullet above.
...
#### Monthly Restatement Marker (v0.18 — ST-08, BLG-FR-05; field names corrected v0.19 — ST-04, BLG-FE-188)

**Design source:** `docs/design/2026-09-21__release-v9.6/monthly-pnl-restatement-diff/decision_record.md`; field-name correction: `docs/design/2026-09-23__release-v9.7/monthly-pnl-restatement-surfacing/decision_record.md`

> **v0.19 correction:** the v9.6 record specified a nested per-month `restatement` object. The live API (`reports_endpoints.md` v0.13) instead returns flat fields — `snapshotted`, `restated`, `snapshot_realised_pnl_gbp`, `restated_diff_gbp` — always present (not gated on differing), with **no snapshot trade-count field**. The data contract and detail-row scope below are corrected to match; the marker's visual treatment is unchanged from v0.18.

A closed month (calendar month ended, UK time) has an **immutable, read-only snapshot** of its Realised P&L taken at month-end; months already closed when the feature ships are baselined once with their then-current figures (never retroactively "restated"). The in-progress month has no snapshot and never shows a marker. There is **no acknowledge / re-baseline action** in the UI.

- **Marker:** shown when `restated = true`, the **Month** cell displays `"Restated"` with a history-style icon prefix in `text-amber-600 dark:text-amber-400` (plain text + icon, not red — information, not an error). It is a keyboard-operable button (`aria-expanded`) that expands an inline detail row; collapsed by default (`data-testid` `monthly-restated-marker` / `monthly-restatement-detail`).
- **Detail row:** a single **Realised P&L** metric row — As reviewed (`snapshot_realised_pnl_gbp`, {`snapshot_date`}) | Now (`realised_pnl_gbp`) | Change (`restated_diff_gbp`). Change values use the standard P&L tone rule and the canonical number format (`design_system.md` §Number and Currency Formatting). There is no Trades row — the API returns no snapshot trade count to compare against. Existing expandable-row pattern; default animation only (≤ 200 ms).
- **Trade-count-only restatement:** `restated` can be `true` while `restated_diff_gbp = 0.00` (only the month's trade count changed). When this occurs (`restated = true` and `restated_diff_gbp = 0`), a muted line appears beneath the Realised P&L metric: `"Trade count for this month has also changed since it was reviewed."` No exact before/after count is shown.
- **API:** each closed month carries `snapshotted` (bool), `restated` (bool), `snapshot_realised_pnl_gbp` (float | null), `restated_diff_gbp` (float | null) — flat fields, always present for closed months (`reports_endpoints.md` v0.13). The in-progress month has `snapshotted = false` and the remaining three fields `false`/`null`. Persisting snapshots uses the `monthly_pnl_snapshots` table (`data_model.md` DS-20).
