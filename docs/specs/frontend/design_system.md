...
**Status:** Canonical
**Version:** 1.17
**Last Updated:** 2026-09-15 (v9.4 sprint execution — ST-27/BLG-UX-04: Toast Notification Timing's non-conforming-screens inventory completed — 8 non-conforming call sites across 5 files, documentation only); prior — 2026-09-15 (v9.4 sprint execution — ST-14/BLG-SPEC-137: new GBP-basis FX-conversion display pattern documented in §Consistency Rules); prior — 2026-09-14 (v9.4 design gate — ST-27/BLG-UX-04: new Toast Notification Timing standard, severity-based durations); prior history retained — see prior entries in version control
...
**v1.17 (ST-27, EPIC-06, v9.4, BLG-UX-04):** Toast Notification Timing (§Shared UI Components) — completed the non-conforming-screens inventory deferred at v1.15's design gate. Read all 17 real `toast.*()` call sites across 8 files; found 8 non-conforming (7 missing a severity-appropriate `duration` override on `toast.error(...)` calls, 1 carrying an incorrect explicit duration on an info toast). Documentation only — no call-site fix in this cycle, per this story's own AC.
...
| `Positions.js` (+ `PositionCard.js`, rendered on this screen) | 1 (`toast.success`, stop-updated confirmation) | 3 (`toast.error` ×3 — stop-update failure ×1 in `Positions.js`, mark-reviewed failure ×2 duplicated across `Positions.js` and `PositionCard.js`) | Same gap; also a pre-existing duplicate-message code smell (identical "Failed to mark position as reviewed" string in two files) noted here but out of this story's scope to fix. |
...
| `ConfirmationModal.js` (shared component, multiple screens) | 2 (undo-window toast — explicit `duration: durationSeconds * 1000`, defaults to 5s, matches the Undo-actionable row exactly; post-undo `toast.success` confirmation — 4s default, correct for success) | 0 | Already conforms — the undo-window mechanism was built with its own explicit duration before this standard existed, and happens to match it. |

**Net finding:** 8 non-conforming error-severity call sites across 5 files (`Layout.js` is a duration-value error, not a missing-override gap; the other 7 are all `toast.error(...)` calls with no severity override). 0 files require a *new* mechanism — `sonner`'s `duration` option already supports everything the standard needs; remediation is a per-call-site prop addition, not new infrastructure. Implementation itself remains a separate, not-yet-filed backlog item (documentation-only this cycle, per this story's own AC) — this inventory is what a future remediation story would work from directly, without re-deriving it. Design source: `docs/design/2026-09-14__release-v9.4/toast-timing-standard/decision_record.md`.
