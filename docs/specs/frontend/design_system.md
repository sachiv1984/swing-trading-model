...
**Status:** Canonical
**Version:** 1.18
**Last Updated:** 2026-09-18 (ST-30, EPIC-04, v9.5, BLG-SPEC-143 — re-confirmed the v9.1 ST-29 empty-state-pattern consolidation still holds: swept all 18 page specs referencing empty states, found no re-derivation of the shared `DataState` mechanism (all page-specific copy correctly follows the v1.8 microcopy pattern rather than redefining it). Found 5 headings with a stray trailing period across `notifications.md`/`trade_plan.md`/`watchlist.md`; 3 were pure spec staleness (shipped code already correct, spec corrected to match) and 2 are genuine shipped-code violations, filed as `BLG-FE-178`/`BLG-FE-179` — no shipped UI change made this story, per its own design-gate scoping); prior — 2026-09-15 (v9.4 sprint execution — ST-27/BLG-UX-04: Toast Notification Timing's non-conforming-screens inventory completed); prior — 2026-09-15 (v9.4 sprint execution — ST-14/BLG-SPEC-137: new GBP-basis FX-conversion display pattern documented in §Consistency Rules); prior history retained — see prior entries in version control
...
- **Motion-vs-contrast guideline for text-element entrance animations (v1.12, ST-05, EPIC-02, v9.2, BLG-SPEC-134):** framer-motion opacity fade-ins (`initial={{opacity: 0}}` → `animate={{opacity: 1}}`, used on `PageHeader.js` and other page-level `motion.div` containers) mean any text element inside one renders at transiently-reduced contrast until it reaches full opacity. Decision: this is an accepted, intentional trade-off, not a defect to eliminate — the motion itself is a deliberate design element (signals page-load completion, gives the UI a less abrupt feel). Rather than requiring text elements to skip opacity animation (which would remove the effect from every page-level header), the guideline is:
  ...
  - **Known non-compliant instances (as of v1.12):** four components stagger text-element opacity fades in a way not confirmed to stay under the 500ms combined ceiling — but not all via the same mechanism, so this is two distinct failure modes, not one:
    - **Index-scaled delay over a list with no fixed upper bound:** `src/pages/SystemStatus.js` (`delay: index * 0.02` / `index * 0.05` over its tests/validations arrays — no cap mechanism of any kind) has no length limit at all; `src/pages/Signals.js` (`delay: index * 0.05` over `filteredSignals`, which is capped via `.slice(0, topN)`) does have a cap operation, but `topN` is a user-editable value with only a `min` enforced and no `max` — so unlike a fixed literal cap, its effective ceiling is not bounded by the component itself.
