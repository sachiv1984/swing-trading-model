...
**Status:** Canonical
**Version:** 1.22
**Last Updated:** 2026-09-23 (ST-24, EPIC-06, v9.6, BLG-SPEC-144 — new §Canonical Chart Data Palette added under §Accessibility, cross-referenced from the existing chart contrast checklist); prior — 2026-09-21 (v9.6 design gate — ST-05/BLG-FE-181 empty-state next-action rule added to §Data States; ST-06/BLG-FE-182 new §Number and Currency Formatting in §Consistency Rules); prior — 2026-09-18 (ST-42, EPIC-06, v9.5, BLG-FE-176 — all 9 non-conforming toast call sites brought into line with the Toast Notification Timing standard; non-conforming-screens table marked historical); prior history retained — see prior entries in version control
...
**v1.22 (ST-24, EPIC-06, v9.6, BLG-SPEC-144):** added the **Canonical Chart Data Palette** subsection (§Accessibility, immediately after the v1.6 chart contrast checklist) — an 8-colour categorical palette (Okabe–Ito, 2008) documented with its source and CVD-type rationale, giving the existing checklist's "each series colour meets contrast" and "no two adjacent series rely on hue alone" rules a concrete default to check against, where none existed before. Cross-referenced from this document only (spec debt, `BLG-SPEC-144`) — no chart component adopts it in this story; adoption is a UI change requiring its own design-gate pass (noted as a follow-up, not filed as a new backlog item since the existing `BLG-SPEC-144` item itself already covers "spec + adoption is separate").
...
**Known non-compliant instances (pending their own follow-up implementation item, `BLG-FE-156`, not fixed by this decision alone):** `WatchlistModal.js`, `ExportModal.js`, `WidgetLibrary.js` — found hardcoding `bg-slate-900 ... text-white` unconditionally at the v8.5 dark/light contrast audit (ST-13). Sequencing note: their conversion should not land before `bg-popover`/`text-popover-foreground` are registered in `tailwind.config.js` (this cycle's ST-04/BLG-FE-147), to avoid reproducing the "empty CSS rule" failure mode BLG-FE-147 exists to close. `PositionEntryModal.js` (originally in this list) was found to be dead/unreachable code and removed from the codebase entirely at v8.8 (ST-16, `BLG-FE-159`) rather than converted — no longer applicable here.
...
**Non-conforming screens — HISTORICAL, all 9 sites now conform (fixed ST-42, EPIC-06, v9.5, BLG-FE-176):** the table below is retained as a record of the original inventory (ST-27, EPIC-06, v9.4, BLG-UX-04 — every real toast-creation call site in `src/` read directly, 18 call sites across 8 files) and its resolution; it no longer reflects current non-conformance. All 9 sites identified below now carry the correct duration: `Layout.js`'s ⌘K hint had its incorrect `duration: 8000` removed (reverts to the 4s info default, correct for its ~62-char message); the other 8 `toast.error(...)` call sites each gained an explicit `duration: 8000` second-argument option. `PositionCard.js`'s and `Positions.js`'s duplicate-message code smell (noted in the original inventory) was left as-is — out of this story's scope, unchanged by this fix.

| Screen / file | Conforming call sites | Non-conforming call sites (as originally inventoried) | Why (historical) |
|----------------|------------------------|------------------------------|-----|
| `Layout.js` (app shell) | — | 1 (`toast.info`, ⌘K search hint) | Explicit `duration: 8000` on an info toast whose ~62-char message is under the 80-char floor — should be the 4s info default, not 8s. The only site where a *wrong* explicit duration existed; every other non-conforming site was simply missing an override, not carrying an incorrect one. |
| `Settings.js` | 1 (`toast.success`, "Settings saved successfully") | 2 (`toast.error` ×2 — form-validation and save-failure messages) | Both errors rendered at the 4s default; standard calls for 8s or manual dismiss. |
| `Signals.js` | 2 (`toast.success` dismiss confirmation; `toast.info` "Already on your watchlist") | 2 (`toast.error` ×2 — add-to-watchlist failure, status-update failure) | Same 4s-default-on-error gap. |
| `Positions.js` | 1 (`toast.success`, stop-updated confirmation) | 2 (`toast.error` ×2 — stop-update failure, mark-reviewed failure) | Same gap. |
| `PositionCard.js` (rendered on the Positions screen) | — | 1 (`toast.error`, mark-reviewed failure) | Same gap; also a pre-existing duplicate-message code smell — this string is identical to `Positions.js`'s own mark-reviewed-failure message, hardcoded independently in both files — noted here but out of this story's scope to fix. |
| `TradeEntry.js` | 2 (`toast.success` plan-linked; `toast.info` no-match-logged) | 0 | No error-severity toast on this screen — nothing to bring into line. |
| `useWatchlistModal.js` (used from Signals/Watchlist surfaces) | 1 (`toast.success`, "kept on watchlist") | 1 (`toast.error`, "Failed to keep ⟨ticker⟩") | Same gap. |
| `ConfirmationModal.js` (shared component, multiple screens) | 2 (undo-window toast, `toast.custom(...)` — explicit `duration: durationSeconds * 1000`, defaults to 5s, matches the Undo-actionable row exactly; post-undo `toast.success` confirmation — 4s default, correct for success) | 0 | Already conformed — the undo-window mechanism was built with its own explicit duration before this standard existed, and happened to match it. |

**Net finding (resolved):** 9 non-conforming call sites across 6 files (`Layout.js` was a wrong-duration-value error; the other 8 were `toast.error(...)` calls with no severity override at all — summing the table above: `Layout.js` 1 + `Settings.js` 2 + `Signals.js` 2 + `Positions.js` 2 + `PositionCard.js` 1 + `useWatchlistModal.js` 1 = 9). 0 files required a *new* mechanism — `sonner`'s `duration` option already supported everything the standard needed; remediation was a per-call-site prop addition, not new infrastructure. Design source: `docs/design/2026-09-14__release-v9.4/toast-timing-standard/decision_record.md`.
...
- **Canonical Chart Data Palette (v1.22, ST-24, EPIC-06, v9.6, BLG-SPEC-144):** the checklist above (v1.6) requires new/modified charts to pass a contrast + hue-alone check but named no default palette to check against — each chart component invented its own (see `AllocationChart.js`'s existing 6-colour set as one example). This entry fixes that gap with a canonical categorical palette, adopted from a recognised colour-blind-safe source rather than picked ad hoc:

  **Source:** Okabe, M. & Ito, K. (2008), *Color Universal Design (CUD) — How to Make Figures and Presentations that are Friendly to Colorblind People*. Designed and validated to remain distinguishable under the three common forms of colour-vision deficiency (protanopia, deuteranopia, tritanopia); in wide use as a default categorical palette (e.g. IBM Design Language, matplotlib/seaborn `"colorblind"`).
  ...
  | 8 | Black (neutral/reference only — not used as a data series colour on the dark theme's near-black background) | `#000000` |

  **Usage rule:** for a new or modified chart with ≤7 data series, assign colours from this table in order 1–7 (skip 8). For a chart needing more than 7 distinguishable series, do not extend this table by inventing additional hues — fall back to a secondary channel (pattern/dash style, direct labelling, or splitting into multiple charts) per the existing "no hue alone" rule above, since colour-only distinction degrades for CVD users well before 8 categories regardless of palette.

  **Per-theme contrast:** this table gives hue, not final on-canvas colour — each value must still individually pass the v1.6 checklist's ≥3:1 non-text contrast check against both the light-theme and dark-theme chart background before use (a palette validated for CVD-distinguishability is not automatically validated for contrast against this app's specific backgrounds).

  **Adoption status (spec debt, out of scope for this story):** no existing chart component (`PnLBarChart.js`, `PortfolioChart.js`, `AllocationChart.js`, `AiSpendTrendChart.js`, `WinRateChart.js`, and others under `src/components/charts/`, `src/components/reports/`, `src/components/analytics/`) has been migrated to this table yet — each currently uses its own hand-picked hex values (e.g. `AllocationChart.js`'s `['#22d3ee', '#a78bfa', '#f472b6', '#fb923c', '#4ade80', '#facc15']`), none audited against the CUD source. Migrating a chart to this palette is a visible UI change and requires its own design-gate pass per this story's own scope note — this entry establishes the target, it does not migrate any component.
- **Motion-vs-contrast guideline for text-element entrance animations (v1.12, ST-05, EPIC-02, v9.2, BLG-SPEC-134):** framer-motion opacity fade-ins (`initial={{opacity: 0}}` → `animate={{opacity: 1}}`, used on `PageHeader.js` and other page-level `motion.div` containers) mean any text element inside one renders at transiently-reduced contrast until it reaches full opacity. Decision: this is an accepted, intentional trade-off, not a defect to eliminate — the motion itself is a deliberate design element (signals page-load completion, gives the UI a less abrupt feel). Rather than requiring text elements to skip opacity animation (which would remove the effect from every page-level header), the guideline is:
  - The bound is **total time-to-full-opacity — `delay` + `duration` combined — not `duration` alone.** A single, non-staggered fade (no `delay`, Framer Motion's default tween of ~0.3–0.5s) stays comfortably under a 500ms ceiling. A **staggered list** (each item's `delay` a function of its index, e.g. `delay: index * 0.05`) does not automatically comply — for a long enough list, the last item's own `delay + duration` can exceed 500ms even though no single item's `duration` was changed. Do not introduce any opacity-animated, text-bearing element (staggered or not) whose own total time-to-full-opacity exceeds 500ms.
  - **Known non-compliant instances:** none remaining as of v1.19 — all 4 instances tracked since v1.12 were brought into compliance at `2026-09-15__release-v9.5` (ST-41, `BLG-FE-175`), per the target release and remediation approach fixed in advance at `docs/design/2026-09-14__release-v9.4/motion-timing-target-release/decision_record.md`:
    - `src/pages/SystemStatus.js` — both index-scaled delays (`tests` list, `validations` list) now use a fixed stagger cap (`delay: Math.min(index, N) * step`, `N` = 9 for the `0.02` step / `N` = 3 for the `0.05` step) plus an explicit `duration: 0.3`, verified against the component's actual value rather than assumed. Max combined: `9×0.02 + 0.3 = 0.48s` and `3×0.05 + 0.3 = 0.45s`, both ≤500ms regardless of list length.
    - `src/pages/Signals.js` — `filteredSignals` stagger now uses the same fixed cap (`delay: Math.min(index, 3) * 0.05`) plus explicit `duration: 0.3` (`0.15 + 0.3 = 0.45s` max). `topN` remains user-editable with only a `min` enforced, per the decision record's own confirmation that this was never itself the defect.
    - `src/pages/Reports.js` — all 4 hardcoded per-card delays (`0.05`–`0.2`) now carry an explicit `duration: 0.3` (max: `0.2 + 0.3 = 0.5s`, at but not over the ceiling).
    - `src/components/dashboard/widgets/RecentTradesWidget.js` — its already-bounded stagger (`slice(0, 5)`, max delay `0.2`) now carries an explicit `duration: 0.3` (`0.2 + 0.3 = 0.5s`).
    - All 4 components' explicit `duration: 0.3` was cross-checked against `framer-motion`'s actual default tween duration (`node_modules/framer-motion/dist/cjs/index.js`, `defaultDuration = defaultTransition.duration || 0.3`) for this project's installed version (12.43.0) — the assumption in the target-release decision record is confirmed correct, and making it explicit removes any dependency on that default going forward.
...
### Currency-Basis Correctness Pattern for US-Market Positions (ST-14, BLG-SPEC-137, v9.4)

**Why this is named explicitly:** two independent currency-basis bugs shipped in the same release (`v8.9`), before either fix noticed the other. Both involve a US-market position's form value and `fx_rate_used`, but they are **opposite failure modes** — do not treat them as one converged rule. Check which mode applies before fixing or building a new display:

**Mode 1 — a value is GBP-converted but the display claims it is native currency.**
- Precedent: `positions.md`'s Trail Stop tile (`DEV-EPIC01-ST02-01`, P0, `BLG-BE-103`). Canonical requirement was "Native currency, 2dp," but `current_trailing_stop` (already GBP-converted server-side) was rendered next to the native currency symbol for US-market positions — two numerically different values both labelled "$".
- **Fix pattern:** do not divide by `fx_rate_used` to "correct" this — the field is already GBP-basis and the label is what's wrong. Surface a genuinely native-currency field instead (the resolution added a dedicated `current_trailing_stop_native` field and rendered that). Applies when the spec calls for a native-currency display but the value in hand has already had FX conversion applied to it somewhere upstream.

**Mode 2 — a value should be GBP-basis but no FX conversion was applied at all.**
- Precedent: `trade_plan.md`'s R at Risk (`DEV-v8.9-ST05-02`, P2, `BLG-FEAT-91`) — implemented as a raw `(entry − stop) × shares` with no FX conversion for any market, contradicting its own spec wording ("FX-converted for US market").
