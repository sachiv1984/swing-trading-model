...
**v1.16 (ST-14, EPIC-04, v9.4, BLG-SPEC-137):** added the GBP-Basis FX-Conversion Display Pattern subsection (§Consistency Rules) — names the recurring currency-basis bug class (a GBP-basis financial figure displayed without applying `fx_rate_used` conversion for US-market positions) that hit twice independently in the same release (v8.9): `DEV-EPIC01-ST02-01` (Trail Stop tile, `positions.md`) and `DEV-v8.9-ST05-02` (R at Risk, `trade_plan.md`). Documents the pattern both fixes converged on so a third instance is caught before shipping rather than after. No shipped UI change — pure spec debt.
...
### GBP-Basis FX-Conversion Display Pattern (ST-14, BLG-SPEC-137, v9.4)

**Rule:** Any GBP-basis financial figure derived from a form that can represent a US-market position must read `fx_rate_used` off the relevant sizing/portfolio response and divide the raw (native-currency) figure by it before display. Do not assume a figure is already GBP-basis just because its label, column header, or currency symbol says so — check the underlying field.

**Why this is named explicitly:** the same root-cause bug shipped twice, independently, in the same release (`v8.9`) before either fix noticed the other:
- `positions.md`'s Trail Stop tile (`DEV-EPIC01-ST02-01`, P0, `BLG-BE-103`) — `current_trailing_stop` was GBP-converted but rendered next to the native currency symbol for US-market positions. Resolved by returning a dedicated `current_trailing_stop_native` field and rendering that instead.
- `trade_plan.md`'s R at Risk (`DEV-v8.9-ST05-02`, P2, `BLG-FEAT-91`) — implemented with no FX conversion at all, contradicting its own spec wording. Resolved by reading `fx_rate_used` from the `POST /portfolio/size` response and dividing by it for US-market plans, matching `TradeEntry.js`'s existing `costs.totalRisk` convention exactly.

Both fixes independently converged on the same correct pattern. Before shipping any new GBP-basis display of a value that can represent a US-market position, confirm which of the two failure modes above applies to it (wrong currency shown vs. no conversion applied) and follow the `fx_rate_used` convention rather than re-deriving the fix from scratch.
