...
**Status:** Canonical
**Version:** 1.19
**Last Updated:** 2026-09-18 (ST-41, EPIC-06, v9.5, BLG-FE-175 — all 4 motion-timing known-non-compliant components brought under the 500ms ceiling and removed from the known-non-compliant list); prior — 2026-09-18 (ST-30, EPIC-04, v9.5, BLG-SPEC-143 — re-confirmed the v9.1 ST-29 empty-state-pattern consolidation still holds, 2 genuine shipped-code violations filed as `BLG-FE-178`/`BLG-FE-179`); prior — 2026-09-15 (v9.4 sprint execution — ST-27/BLG-UX-04: Toast Notification Timing's non-conforming-screens inventory completed); prior history retained — see prior entries in version control
...
**Non-conforming screens (inventoried ST-27, EPIC-06, v9.4, BLG-UX-04):** every real toast-creation call site in `src/` was read directly — **18 call sites across 8 files** (`grep`-ing only `toast.(success|error|warning|info|message)(` finds 17 of these; `ConfirmationModal.js`'s undo-window toast uses `toast.custom(...)` and is the 18th, easy to miss with a severity-name-only grep — found here by reading the file directly rather than trusting the narrower pattern). **Two** call sites pass an explicit `duration` (not one): `Layout.js`'s ⌘K hint (`duration: 8000`, incorrectly) and `ConfirmationModal.js`'s undo-window toast (`duration: durationSeconds * 1000`, correctly — see its own row below). Every other call site relies on `sonner`'s bare 4s default regardless of severity — meaning every `toast.error(...)` call in the app is non-conforming today, since this table's Error row (8s/manual-dismiss) did not exist before this standard. This is the expected starting state for a newly-created standard with "no shipped call-site change this cycle" (per the v1.15 changelog entry above), not a regression.

| Screen / file | Conforming call sites | Non-conforming call sites | Why |
|----------------|------------------------|------------------------------|-----|
| `Layout.js` (app shell) | — | 1 (`toast.info`, ⌘K search hint) | Explicit `duration: 8000` on an info toast whose ~62-char message is under the 80-char floor — should be the 4s info default, not 8s. The only site where a *wrong* explicit duration exists; every other non-conforming site is simply missing an override, not carrying an incorrect one. |
| `Settings.js` | 1 (`toast.success`, "Settings saved successfully") | 2 (`toast.error` ×2 — form-validation and save-failure messages) | Both errors render at the 4s default; standard calls for 8s or manual dismiss. |
...
| `ConfirmationModal.js` (shared component, multiple screens) | 2 (undo-window toast, `toast.custom(...)` — explicit `duration: durationSeconds * 1000`, defaults to 5s, matches the Undo-actionable row exactly; post-undo `toast.success` confirmation — 4s default, correct for success) | 0 | Already conforms — the undo-window mechanism was built with its own explicit duration before this standard existed, and happens to match it. |

**Net finding:** 9 non-conforming call sites across 6 files (`Layout.js` is a wrong-duration-value error; the other 8 are all `toast.error(...)` calls with no severity override at all — summing the table above: `Layout.js` 1 + `Settings.js` 2 + `Signals.js` 2 + `Positions.js` 2 + `PositionCard.js` 1 + `useWatchlistModal.js` 1 = 9). 0 files require a *new* mechanism — `sonner`'s `duration` option already supports everything the standard needs; remediation is a per-call-site prop addition, not new infrastructure. Implementation itself is filed as `BLG-FE-176` (documentation only this cycle, per this story's own AC) — this inventory is what that story works from directly, without re-deriving it. Design source: `docs/design/2026-09-14__release-v9.4/toast-timing-standard/decision_record.md`.
