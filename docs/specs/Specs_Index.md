...
**Identified:** 2026-03-24 (delivery verification 2026-03-21__release-v2.2)
**Status:** ✅ RESOLVED — 2026-09-03 (QA & Testing Owner direct action, post-ship closure 2026-08-21__release-v9.0 outstanding-actions resolution). `tests/test_health_response_schema.py` added — 14 tests deriving acceptance criteria directly from `health_endpoints.md`'s documented response schemas: `SC-HEALTH-01a` (`GET /health` — top-level keys, `status`/`db` enum values, timestamp null-or-string typing, `external_apis` per-provider shape, `ai_journal` both shapes including the DB-failure path) and `SC-HEALTH-01b` (`GET /health/database` — all 7 documented fields, types, the 256MB Render free-tier constant, `ok`/`warning`/`error` status transitions, and the query-failure fallback shape). All 14 pass against the current implementation (`backend/services/health_service.py`); full backend suite (1341 passed, 10 skipped) re-verified with no regressions. Open ~24 cycles (2026-03-24 → 2026-09-03).
**Owner:** QA & Testing Owner + Infrastructure & Operations Owner
**Resolution:** `docs/specs/api_contracts/health_endpoints.md` was already current (v1.2+, no spec gap) — only the missing automated scenario itself was the outstanding item. Mock-based unit tests against `get_operational_health()`/`get_db_size_info()` directly (per `SBX-NO-LIVE-DB`, `shared_standards.md §16.16` — no live DB connection required or available in this sandbox), consistent with the existing pattern in `tests/test_health_extensions.py`.
...
**Status:** ✅ RESOLVED — 2026-09-03 (QA & Testing Owner direct action, post-ship closure 2026-08-21__release-v9.0 outstanding-actions resolution)
**Owner:** QA & Testing Owner
...
**Resolution (2026-09-03):** Re-investigated the "staging execution" premise itself — this repo's entire Playwright E2E suite (`playwright.config.js`) runs against a local build+serve webServer with mock-layer `page.route()` interception, not live staging; V-CHART-05a/05b/05c had always named their own `Scenario ref` as `SC-CHART-IX-05a`/`SC-CHART-IX-05b`, which already exist in `tests/e2e/chart-interactivity.spec.js` and were confirmed passing in a real local Playwright run (20/21 specs in the file passed; 1 unrelated failure). V-CHART-05a/05b's underlying concern (bar tooltip renders with correct data, zero-count buckets render) is covered — using this codebase's own deliberate structural/no-crash convention for Recharts tooltip testing (matching `SC-CHART-IX-04a`'s established approach), not literal hover-triggered text assertion. V-CHART-05c (tooltip edge-cursor repositioning) has no dedicated automated check — accepted as a residual, low-value gap: it exercises Recharts' own built-in `Tooltip` positioning, not custom application logic, and the same library behaviour is exercised without incident by every other hover-tested chart in the suite. Full detail: `docs/testing/staging_visual_test_script_ST-06.md` §4 (updated in the same session). Open ~24 cycles (2026-03-30 → 2026-09-03) — the entries never needed live staging at all; the "staging-blocked" framing was itself stale once the mock-layer E2E convention was established for this codebase, and no closure between then and now had re-examined that premise.
...
- **TSG-v22-02** (`SC-HEALTH-01`, `GET /health` schema validation scenario, §10.2 above): at post-ship closure time, confirmed still genuinely Open (no `SC-HEALTH-01` scenario file existed anywhere in `tests/`; not a stale-but-actually-resolved case) — recorded as an Outstanding Action outside that engine's write scope. **Resolved 2026-09-03, same-day follow-up (QA & Testing Owner direct action):** `tests/test_health_response_schema.py` authored (14 tests, `SC-HEALTH-01a`/`SC-HEALTH-01b`), all passing. Marked ✅ RESOLVED.
- **TSG-v23-01** (`V-CHART-05a/b/c`, R-Multiple chart tooltip staging visual scenarios, §10.3 above): at post-ship closure time, confirmed still genuinely Open — recorded as an Outstanding Action outside that engine's write scope. **Resolved 2026-09-03, same-day follow-up (QA & Testing Owner direct action):** re-investigated the "staging execution" premise itself and found it stale — this repo's E2E suite runs against a local build, not live staging; the named scenario refs (`SC-CHART-IX-05a/05b`) already existed and were confirmed passing in a real local Playwright run. Marked ✅ RESOLVED (V-CHART-05c accepted as a residual low-value gap — see §10.3).
...
## 44. Test Coverage Gaps — v9.4 (2026-09-14__release-v9.4)

Identified during delivery verification (`verification_report.md §6`): **0 new test scenario gaps this cycle** — every EPIC's `test_scenarios` array was cross-referenced against its `qa_evidence_EPIC-xx.md` "Scenarios run" field and confirmed executed; EPIC-04 correctly recorded `not_applicable` (empty `test_scenarios`, no frontend-visible AC — all 5 stories were documentation/decision-record work). Table is N/A per §6.
