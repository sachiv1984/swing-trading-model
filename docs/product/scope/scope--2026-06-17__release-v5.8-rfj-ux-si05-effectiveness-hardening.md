**Owner:** Head of Specs Team
**Class:** Planning Document (Class 4)
**Status:** Published
**Release:** v5.8
**Cycle:** 2026-06-17__release-v5.8
**Last Updated:** 2026-06-17

---

# Scope Document — v5.8

**Theme:** RFJ UX Design Completion, SI-05 Effectiveness Review & Production Hardening

---

## Items in scope

| S2-ID | BLG-ID | Title | Type | Effort | Conditional |
|-------|--------|-------|------|--------|-------------|
| S2-01 | BLG-FE-64 | RFJ design review pre-brief | Frontend/UX | XS | No |
| S2-02 | BLG-FE-41 | Red Flag Journal visual design review | Frontend/UX | M | No |
| S2-03 | — | FRONTEND_URL production env var configuration | Ops | XS | No |
| S2-04 | BLG-GOV-101 | Governance model complexity assessment | Governance | M | No |
| S2-05 | BLG-GOV-112 | SI-05 digest weekly cadence review | Governance | S | Yes — gate 2026-07-04 |
| S2-06 | BLG-GOV-115 | SI-05 digest actionability metric definition | Governance | S | Yes — gate 2026-07-04 |
| S2-07 | BLG-OPS-59 | SI-05 service production p99 latency review | Ops | S | Yes — gate 2026-07-04 |

**Total:** 7 items (4 firm, 3 conditional)

---

## Items explicitly deferred

| Item | Reason |
|------|--------|
| BLG-FEAT-25 PT-04 Setup Quality Score | Gate: 20+ closed trades (currently 13) |
| SI-02 frontend (BLG-GOV-107) | Gate: 20+ closed trades with linked trade_plans |
| SI-04 strategy version comparison | Later horizon |
| Arc 4 PO-02/PO-03/PO-04 | Gate: 6+ months AI journals (~Oct 2026) |
| Arc 6 PS-01–PS-05 | Gate: 100+ trades; 12–18 months history |

---

## Supersession note

*(Completed at Post-Ship Closure — superseded by [scope--2026-06-17__release-v5.8-rfj-ux-si05-effectiveness-hardening.md v1.0 — authoritative as-shipped]*
