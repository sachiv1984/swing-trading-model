...
- **Assessment: real completeness gap for Arc 6/SI-02's `win_rate_by_setup_type` query.** Not fixed directly in this audit — the right fix (should `setup_type` become a required field at submission, or should an explicit "Unclassified" enum value be added so it groups predictably rather than nulling silently?) is a product/UX decision outside this audit's S-effort scope, not a mechanical code change. Filed as `BLG-QA-149` (see §5) for Product Owner / Frontend Specifications & UX Documentation Owner disposition.

## 5. Gaps Filed

| Ref | Field | Gap | Disposition |
|-----|-------|-----|-------------|
| BLG-QA-149 | `trade_plans.setup_type` | No default/required-field guarantee outside the linked-signal pre-population path; plans created any other way save `null`, undercounting future Arc 6 `win_rate_by_setup_type` analysis | Filed — Product Owner / Frontend Specifications & UX Documentation Owner to decide requirement-vs-default treatment |
