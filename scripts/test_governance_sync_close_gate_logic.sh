...
#
# ST-16 (BLG-QA-165, EPIC-03, v9.5): is_story_done() now sources the real
# implementation from scripts/governance_sync_lib.sh — the same function
# .github/workflows/governance_sync.yml's "Update State and Close Issues"
# step sources — instead of maintaining its own hand-copied duplicate.
# CYCLES_BASE is passed through as that function's first argument, so this
# script can still point it at a disposable fixture directory instead of a
# real cycle folder.
#
...
would_close() {
  [ "$(is_story_done "$CYCLES_BASE" "$1" "$2")" = "yes" ] && echo "close" || echo "skip"
}
...
RESULT_1=$(is_story_done "$CYCLES_BASE" "$FAKE_CYCLE" "ST-99")
...
RESULT_3=$(is_story_done "$CYCLES_BASE" "$FAKE_CYCLE" "ST-01")
...
RESULT_4=$(is_story_done "$CYCLES_BASE" "" "ST-01")
